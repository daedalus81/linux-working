...
bool soidle_can_enter(int cpu)
{
	...
	if (cpu == 0) {
		...
		if (!clkmgr_idle_can_enter(soidle_condition_mask,
					soidle_block_mask)) {
			reason = BY_CLK;
			goto out;
		}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < NR_GRPS; i++) {
		p += sprintf(p,
				"[%02d]soidle_condition_mask[%-8s]=0x%08x"
...
#else
	for (i = 0; i < nr_cpu_ids; i++) {
		p += sprintf(p, "dpidle_cnt[%d]=%lu, slidle_cnt[%d]=%lu,"
				"rgidle_cnt[%d]=%lu\n",	i, dpidle_cnt[i],
				i, slidle_cnt[i], i, rgidle_cnt[i]);
	}
#endif
