...
static unsigned int mt_eint_get_mask(unsigned int eint_num)
{
	...
	if (eint_num < EINT_AP_MAXNUMBER)
		...
		dbgmsg("Error in %s [EINT] num:%d is larger"
				"than EINT_AP_MAXNUMBER\n",
				__func__, eint_num);
		return 0;
	}
...
static unsigned int mt_can_en_debounce(unsigned int eint_num)
{
	...
	if (EINT_FUNC.deb_time[eint_num] != 0 && sens != MT_EDGE_SENSITIVE)
		...
		dbgmsg("Can't enable debounce of eint_num:%d,"
				"deb_time:%d, sens:%d\n",
				eint_num, EINT_FUNC.deb_time[eint_num],
				sens);
		return 0;
	}
}
...
void mt_eint_set_hw_debounce(unsigned int eint_num, unsigned int ms)
{
	...
	if (eint_num >= MAX_HW_DEBOUNCE_CNT)
		...
		/* step 2.4: Delay a while (more than 2T) to wait
		 * for hw debounce enable work correctly */
		udelay(500);
		...
		/* step 2.6: Delay a while (more than 2T) to wait
		 * for hw debounce counter reset work correctly */
		udelay(500);
	}
...
#if EINT_USE_PRECISE_HRTIMER
static enum hrtimer_restart mt_eint_hrtimer_handler(struct hrtimer *eint_timer)
{
	//todo:
	EINT_HR_TIMER_T *pr_eint_hrtimer = NULL;

	pr_eint_hrtimer = container_of(eint_timer, EINT_HR_TIMER_T, rTimer);

	if (pr_eint_hrtimer->eint_num >= EINT_MAX_CHANNEL) {
		printk("Fatal error !!!!!!!!!!!!!! \n");
	...
	printk("eint %d hr timer 0x%x receive callback\n",
			pr_eint_hrtimer->eint_num, pr_eint_hrtimer);
#endif

	mt_eint_timer_event_handler(pr_eint_hrtimer->eint_num);

#if EINT_TEST_LATENCY
	printk("eint %d handler done\n", pr_eint_hrtimer->eint_num);
#endif

	//spin_lock(&eint_lock);
	pr_eint_hrtimer->eint_num = EINT_MAX_CHANNEL;
...
static void mt_eint_set_timer_event(unsigned int eint_num)
{
	...
	{
		struct hrtimer *pr_hrtimer = &EINT_FUNC.eint_sw_deb_hr_timer[eint_num].rTimer;
		ktime_t ktime;
...
#if EINT_TEST_LATENCY
			printk("eint %d set timer 0x%x with eint num %d,"
					"but already set before !!!!!!\n",
					eint_num, pr_hrtimer,
					EINT_FUNC.eint_sw_deb_hr_timer[eint_num].eint_num);
#endif
		...
		hrtimer_init(pr_hrtimer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);

		pr_hrtimer->function = mt_eint_hrtimer_handler;
		hrtimer_start(pr_hrtimer, ktime, HRTIMER_MODE_REL);
	}
	//else
#else
	{
...
static irqreturn_t mt_eint_isr(int irq, void *dev_id)
{
	 ...
	 * tirggers.  This is used to prevent system from suspend such that
	 * other drivers or applications can have enough time to obtain their
	 * own wake lock.  (This information is gotten from the power management
	 * owner.)
	 */
				...
				if ((EINT_FUNC.is_deb_en[index] == 1) &&
					...
					if (24 == index) {
						printk("Test latency, 3, dir:0x%x, mode:0x%x, en:0x%x, sel:0x%x\n",
								mt_get_gpio_dir(2), mt_get_gpio_mode(2),
								mt_get_gpio_pull_enable(2),
								mt_get_gpio_pull_select(2));
	    ...
	    irqreturn_t (DEINT_FUNC_PTR) (int, void *))
{
	...
	if (deint_num < DEINT_MAX_CHANNEL) {
		...
		if(EINT_FUNC.eint_func[num] != NULL) {
			printk("[Wrong DEint-%d] has been registered"
					"as EINT pin.!!! \n", deint_num);
			spin_unlock(&eint_lock);
			return;
		}
		...
		if (request_irq(MT_EINT_DIRECT0_IRQ_ID + deint_num,
					DEINT_FUNC_PTR,
					IRQF_TRIGGER_HIGH,
					"DEINT", NULL ))
	...
	if (cur_eint_num >= MAX_HW_DEBOUNCE_CNT)
		...
		switch (dbnc) {
			case 0:
				...
				dbgmsg(KERN_CRIT "ms should not be 0."
						"eint_num:%d in %s\n",
			...
			default:
				deb = 0;
				printk("invalid deb time in the EIN_CON"
						"register, dbnc:%d, deb:%d\n",
						dbnc, deb);
				break;
		}
	}
 ...
 */
static ssize_t cur_eint_soft_set_store(struct device_driver *driver,
...
DRIVER_ATTR(current_eint_soft_set, 0664,
		cur_eint_soft_set_show, cur_eint_soft_set_store);
...
static ssize_t cur_eint_reg_isr_show(struct device_driver *driver, char *buf)
{
	...
	if (cur_eint_num >= MAX_HW_DEBOUNCE_CNT)
		...
		switch (dbnc) {
			...
			default:
				deb = 0;
				printk("invalid deb time in the EIN_CON register"
						"dbnc:%d, deb:%d\n", dbnc, deb);
				break;
		}
	}
	dbgmsg("sens:%d, pol:%d, deb:%d, isr:%p, autounmask:%d\n",
			sens, pol, deb, EINT_FUNC.eint_func[cur_eint_num],
			autounmask);
...
static int __init mt_eint_init(void)
{
...
#if EINT_USE_PRECISE_HRTIMER
 		//init for unused
		EINT_FUNC.eint_sw_deb_hr_timer[i].eint_num = EINT_MAX_CHANNEL;
#if EINT_TEST_LATENCY
		if (24 == i)
	...
	if (request_irq(MT_EINT_IRQ_ID, mt_eint_isr,
				IRQF_TRIGGER_HIGH, "EINT", NULL))
