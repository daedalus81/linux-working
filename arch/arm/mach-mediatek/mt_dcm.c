		...
		unsigned int *pui4_m4u_dcm)
{
	...
	paramTypes = TZ_ParamTypes3(TZPT_VALUE_INPUT, TZPT_VALUE_OUTPUT,
			TZPT_VALUE_OUTPUT);
	param[0].value.a = SMI_DCM;
	param[1].value.a = 0;
	param[1].value.b = 0;
	...
	ret = KREE_TeeServiceCall(dcm_session, TZCMD_DCM_GET_DCM_STATUS,
			paramTypes, param);
	if (TZ_RESULT_SUCCESS != ret)
		...
		unsigned int *pui4_smi_com_clr)
{
	...
	ret = KREE_TeeServiceCall(dcm_session, TZCMD_DCM_GET_DCM_OP_STATUS,
			paramTypes, param);
	if (TZ_RESULT_SUCCESS != ret)
...
void dcm_dump_regs(unsigned int type)
{
	...
	if (type & PER_DCM) {
		...
		dcm_info("[PER_DCM]TOP_CKDIV1(0x%08x), TOP_DCMCTL(0x%08x),"
				"TOP_DCMDBC(0x%08x), INFRA_DCMCTL(0x%08x),"
				"INFRA_DCMDBC(0x%08x), INFRA_DCMFSEL(0x%08x),"
				"DRAMC_PD_CTRL(0x%08x)\n",
				top_ckdiv1, top_dcmctl, top_dcmdbc,
				infra_dcmctl, infra_dcmdbc, infra_dcmsel,
				infra_pd_Ctrl);
	}
...
#endif  /* SUPPORT_MTEE_SMI_DCM_PROT */
		dcm_info("[SMI_DCM]SMI_DCM_CONTROL(0x%08x),"
				"SMI_COMMON_AO_SMI_CON(0x%08x),"
				"MMU_DCM(0x%08x)\n" , smi_dcm_control,
				smi_common_ao_smi_con, mmu_dcm);
	...
	if (type & ISP_DCM) {
		if (subsys_is_on(SYS_ISP)) {
			...
			unsigned int  jpgenc, venc_dcm, venc_cg,
				      smilarb2_dcm_sta, smilarb2_dcm_com;
	...
	if (type & VDE_DCM) {
		if (subsys_is_on(SYS_VDE)) {
			...
			dcm_info("[VDE_DCM]VDEC_DCM_CON(0x%08x),"
					"SMILARB1_DCM_STA(0x%08x),"
					"SMILARB1_DCM_CON(0x%08x),"
					"SMILARB1_DCM_SET(0x%08x)\n",
					vdec, smilarb1_dcm_sta,
					smilarb1_dcm_com, smilarb1_dcm_set);
		} else
...
#if SUPPORT_MTEE_SMI_DCM_PROT
		/* SMI_SECURE_XXX register is protected by MTEE */
		/* Note: driver initialization should not call this function due
		 * to driver iniitializaion sequence */
...
void mt_dcm_init(void)
{
	...
	/*
Note:
1. SMI_SECURE_XXX register is protected by MTEE SMI_DCM is enabled by DCM driver
in MTEE
2. Although initialization sequence for DCM kernel driver and MTEE
driver is not guarantee in kernel, it is ok to make status of SMI_DCM to be set
to "enable" in DCM kernel driver initialization
*/
