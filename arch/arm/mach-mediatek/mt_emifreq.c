...
#define dprintk(fmt, args...)					\
	do {							\
		if (mt_emifreq_debug) {				\
			printk(KERN_INFO "Power/EMI_DFS:" fmt, ##args);	\
		}						\
	} while(0)
...
static ssize_t mt_emifreq_state_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(buffer, "%d", &enabled) == 1) {
		if (enabled == 1)
			...
			printk(KERN_INFO "Power/EMI_DFS:bad argument!!"
...
void mt_emifreq_early_suspend(struct early_suspend *h)
{
	if(mt_emifreq_pause == false) {
		mt_h2l_dvfs_mempll();
		printk(KERN_INFO "Power/EMI_DFS: mt_emifreq_early_suspend\n");
	}
}
...
void mt_emifreq_late_resume(struct early_suspend *h)
{
	...
	{
		mt_l2h_dvfs_mempll();
		printk(KERN_INFO "Power/EMI_DFS: mt_emifreq_late_resume\n");
	}
}
...
static int __init mt_emifreq_init(void)
{
	...
	if (!mt_emifreq_dir)
		...
		mt_entry = proc_create("emifreq_debug",
		...
		mt_entry = proc_create("emifreq_state",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_emifreq_dir,
