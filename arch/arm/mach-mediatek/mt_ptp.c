...
int mt_ptp_log_thread_handler(void *unused)
{
	do {
		...
		ptp_notice("PTP_LOG: (%d) - (%d, %d, %d, %d, %d, %d, %d, %d) -"
				"(%d, %d, %d, %d, %d, %d, %d, %d)\n", \
...
static void PTP_Initialization_01(PTP_INIT_T* ptp_init_val)
{
	...
	// config PTP register
	ptp_write(PTP_DESCHAR, ((((ptp_init_val->BDES)<<8) & 0xff00)
				| ((ptp_init_val->MDES) & 0xff)));

	ptp_write(PTP_TEMPCHAR, ((((ptp_init_val->VCO)<<16) & 0xff0000)
				| (((ptp_init_val->MTDES)<<8) & 0xff00)
				| ((ptp_init_val->DVTFIXED) & 0xff)));

	ptp_write(PTP_DETCHAR,   ((((ptp_init_val->DCBDET)<<8) & 0xff00)
				| ((ptp_init_val->DCMDET) & 0xff)));
	ptp_write(PTP_AGECHAR,   ((((ptp_init_val->AGEDELTA)<<8) & 0xff00)
...
static void PTP_Initialization_02(PTP_INIT_T* ptp_init_val)
{
	...
	ptp_write(PTP_INIT2VALS, ((((ptp_init_val->AGEVOFFSETIN)<<16) & 0xffff0000)
...
static void PTP_Monitor_Mode(PTP_INIT_T* ptp_init_val)
{
	...
	ptp_write(PTP_DCCONFIG,  ((ptp_init_val->DCCONFIG)));
	ptp_write(PTP_AGECONFIG, ((ptp_init_val->AGECONFIG)));
	ptp_write(PTP_TSCALCS,   ((((ptp_init_val->BTS)<<12) & 0xfff000)
...
irqreturn_t mt_ptp_isr(int irq, void *dev_id)
{
	...
	if (PTPINTSTS == 0x1) {// PTP init1 or init2
		...
		if ((temp_0 > 0x4b) && (temp_0 < 0xd3))
			ptp_isr_info("thermal sensor init has not been"
					"completed. (temp_0 = 0x%x)\n", temp_0);
		else {
			temp = ptp_read(PTP_VOP30); // read ptp_volt_0 ~ ptp_volt_3
			ptp_volt_0 = temp & 0xff;
			ptp_volt_1 = (temp>>8) & 0xff;
		...
		if (((temp_ptpen & 0x7) == 0x1) || ((temp_ptpen & 0x7) == 0x5)) { // init 1  || init 2 error handler
			ptp_isr_info("====================================================\n");
			ptp_isr_info("PTP error_1 error_2 (0x%x) : PTPINTSTS =
					0x%x\n", temp_ptpen, PTPINTSTS);
			ptp_isr_info("PTP_SMSTATE0 (0x%x) = 0x%x\n",
					PTP_SMSTATE0, ptp_read(PTP_SMSTATE0));
			ptp_isr_info("PTP_SMSTATE1 (0x%x) = 0x%x\n",
...
static int ptp_probe(struct platform_device *pdev)
{
	...
	mt_ptp_volt_thread = kthread_run(mt_ptp_volt_thread_handler, 0,
			"ptp volt");
 ...
 ****************************/
static ssize_t ptp_offset_show(struct kobject *kobj,
 ...
 ****************************/
static ssize_t ptp_debug_show(struct kobject *kobj, struct kobj_attribute *attr,
		char *buf)
{
	int len = 0;
	char *p = buf;
		...
		struct kobj_attribute *attr, const char *buf, size_t n)
{
	...
	if (sscanf(buf, "%d", &enabled) == 1) {
		if (enabled == 0)
			...
			ptp_notice("bad argument_0!! argument shd be \"0\"\n");
	}
	else
		ptp_notice("bad argument_1!! argument shd be \"0\"\n");
 ...
 ****************************/
static ssize_t ptp_dump_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	int len = 0;
	char *p = buf;
 ...
 ************************/
static ssize_t ptp_cur_volt_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	int len = 0;
	char *p = buf;
 ...
 * show current PTP status
 ***************************/
static ssize_t ptp_status_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	int len = 0;
	char *p = buf;

	p += sprintf(p, "PTP_LOG: (%d) - (%d, %d, %d, %d, %d, %d, %d, %d) - (%d,"
	  	  	  "%d, %d, %d, %d, %d, %d, %d)\n", \
			mtktscpu_get_cpu_temp(), \
			ptp_trasnfer_to_volt(ptpod_pmic_volt[0]), \
			ptp_trasnfer_to_volt(ptpod_pmic_volt[1]), \
 ...
 * show PTP log enable
 ****************************/
static ssize_t ptp_log_en_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return 0;
}
		...
		struct kobj_attribute *attr, const char *buf, size_t n)
{
	...
	if (sscanf(buf, "%d", &enabled) == 1) {
		if (enabled == 1) {
			...
			mt_ptp_log_thread = kthread_run(mt_ptp_log_thread_handler,
					0,
					"ptp logging");
			...
			ptp_notice("bad argument!! argument shd be \"0\" or"
					"\"1\"\n");
		}
	} else
		ptp_notice("bad argument!! argument shd be \"0\" or \"1\"\n");
