			...
			case 1: /* MM */
				...
				if(mm_larb == 0x3) {
					if(smi_port >= ARRAY_SIZE(smi_larb0_port)) {
						printk(KERN_CRIT "[EMI MPU ERROR] Invalidate master ID!"
				...
				break;
			default:
				printk(KERN_CRIT "[EMI MPU ERROR] Invalidate"
						"port ID! lookup bus ID table"
						"failed!\n");
				break;
		}
		return 1;
...
static irqreturn_t mpu_violation_irq(int irq, void *dev_id)
{
	...
	//EMI_MPUT = 0x%x, EMI_MPU(PQR).\n", dbg_s, dbg_t+EMI_PHY_OFFSET,
	//dbg_pqry);
	aee_kernel_exception("EMI MPU",
			"EMI MPU violation.\nEMP_MPUS = 0x%x, EMI_MPUT = 0x%x,"
			"module is %s.\n", dbg_s, dbg_t+EMI_PHY_OFFSET,
			master_name);
#endif
...
#if 0
//Marcos(MT6582): Each hw module has an unique ID. There is no need to use
//notifier function to distinguish different hw module which has the same bus
//ID.
		...
		const char *buf, size_t count)
{
	...
	if (!strncmp(buf, EN_MPU_STR, strlen(EN_MPU_STR))) {
		...
		emi_mpu_set_region_protection(start_addr, end_addr, region,
				access_permission);
		printk(KERN_CRIT "Set EMI_MPU: start: 0x%x, end: 0x%x,"
				"region: %d, permission: 0x%x.\n",
		...
		emi_mpu_set_region_protection(0x0, 0x0, region,
				SET_ACCESS_PERMISSON( NO_PROTECTION,
					...
					NO_PROTECTION,
					NO_PROTECTION));
		printk("set EMI MPU: start: 0x%x, end: 0x%x, region: %d,"
				"permission: 0x%x\n",
				0, 0, region,
				SET_ACCESS_PERMISSON( NO_PROTECTION,
					NO_PROTECTION,
...
void mtk_search_full_pgtab(void)
{
	...
	for(addr=0xC0000000; addr<0xFFF00000; addr+=0x100000)
	{
		...
		if ((pmd_val(*pmd) & PMD_TYPE_MASK) == PMD_TYPE_TABLE) {
			...
			for(; addr_2nd<(addr_2nd_end); addr_2nd+=0x1000) {
				...
				//printk("[EMI MPU] pmd: %x, pte: %x, *pte = %x,
				//addr_2nd = 0x%x, addr_2nd_end = 0x%x\n",
				//(unsigned long)(pmd), (unsigned long)(pte),
				//(unsigned long)pte_val(*(pte)), addr_2nd,
				//addr_2nd_end);
				if(((unsigned long)v_addr & PAGE_MASK) ==
						((unsigned long)pte_val(*(pte)) & PAGE_MASK)) {
					printk("[EMI MPU] Find page entry section at pte: %lx."
							"violation address = 0x%x\n",
			...
			if(((unsigned long)pmd_val(*(pmd)) & SECTION_MASK) ==
				...
				printk("[EMI MPU] Find page entry section at pmd: %lx."
						"violation address = 0x%x\n",
	...
	printk("[EMI MPU] ****** Can not find page table entry!"
			"violation address = 0x%x ******\n", v_addr);
...
static ssize_t pgt_scan_store(struct device_driver *driver, const char *buf, size_t count)
{
	...
	if(value == 1) {
		...
		if(!ret)
			printk(KERN_DEBUG "[EMI MPU] submit workqueue failed,"
					"ret = %d\n", ret);
	}
...
static void emi_axi_dump_info(int aee_ke_en)
{
	...
	if (value & 0x7FFF0000)
	{
		...
		printk(KERN_CRIT "Violation master ID is 0x%x.\n", master_ID);
		printk(KERN_CRIT "Violation type is: AXI_ADR_CHK_EN(%d),"
				"AXI_LOCK_CHK_EN(%d), AXI_NON_ALIGN_CHK_EN(%d)\n",
				(value & (1 << AXI_ADR_CHK_EN)) ? 1 : 0,
				(value & (1 << AXI_LOCK_CHK_EN)) ? 1 : 0,
		...
		if (aee_ke_en)
			aee_kernel_exception("EMI MPU AXI",
					"AXI violation.\EMI_CHKER = 0x%x,"
					"module is %s.\n", value, master_name);
#endif
		// clear AXI checker status
		mt65xx_reg_sync_writel((1 << AXI_VIO_CLR) | readl(IOMEM(EMI_CHKER)),
				EMI_CHKER);
	}
}
...
static int __init emi_mpu_mod_init(void)
{
	...
	mt65xx_reg_sync_writel(readl(IOMEM(DEVAPC0_APC_CON))
			& (0xFFFFFFFF ^ (1<<2)), DEVAPC0_APC_CON);
	mt65xx_reg_sync_writel(readl(IOMEM(DEVAPC0_PD_APC_CON))
			& (0xFFFFFFFF ^ (1<<2)), DEVAPC0_PD_APC_CON);

	mt65xx_reg_sync_writel(ABORT_EMI_BUS_INTERFACE,
			DEVAPC0_D0_VIO_STA_0);
	mt65xx_reg_sync_writel(readl(IOMEM(DEVAPC0_D0_VIO_MASK_0))
			& (0xFFFFFFFF ^ (1<<23)), DEVAPC0_D0_VIO_MASK_0);
	mt65xx_reg_sync_writel(ABORT_EMI, DEVAPC0_D0_VIO_STA_3);
	mt65xx_reg_sync_writel(readl(IOMEM(DEVAPC0_D0_VIO_MASK_3))
	 ...
	 */
#if 1 //#ifndef CONFIG_MT8127_EVB_BOARD
	ret = request_irq(MT_APARM_DOMAIN_IRQ_ID,
			(irq_handler_t)mpu_violation_irq,
			IRQF_TRIGGER_LOW | IRQF_SHARED,
			"mt_emi_mpu",
			&emi_mpu_ctrl);
	if (ret != 0) {
		printk(KERN_CRIT "Fail to request EMI_MPU interrupt err=%d\n", ret);
		return ret;
	}
#endif
	...
	if (ret)
		printk(KERN_CRIT "Fail to create AXI violation"
				"monitor sysfs file.\n");
#endif
