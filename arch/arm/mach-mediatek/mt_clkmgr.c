...
static inline int subsys_disable_internal(struct subsys *sys,
		int force_off, char *name)
{
	int err;
...
static int general_pll_calc_freq(unsigned int *pcw, int *postdiv_idx,
		unsigned int freq, int pcwfbits)
{
...
static int general_pll_set_freq_op(struct pll *pll,
		unsigned int freq, int pcwfbits)
{
	int r;
	unsigned int pcw = 0;
...
static void arm_pll_set_postdiv_op(struct pll *pll, unsigned int postdiv)
{
	...
	clk_writel(con1_addr, (con1_value & ~BITMASK(26:24))
			| BITS(26:24, postdiv));   /* bit[26:24] */
}
...
{
	return pll->ops->vco_calc(pll)
		/ pll_posdiv_map[pll->ops->get_postdiv(pll)];
}
...
static int clk_enable_locked(struct cg_clk *clk)
{
...
#ifdef CLK_LOG
	clk_info("[%s]: Start. grp->name=%s, grp->state=0x%x, clk->mask=0x%x,"
			"clk->cnt=%d, clk->state=%d\n",
	...
	grp->state |= clk->mask;
#ifdef CLK_LOG
	clk_info("[%s]: End. grp->name=%s, grp->state=0x%x, clk->mask=0x%x,"
			"clk->cnt=%d, clk->state=%d\n",
			__func__, grp->name, grp->state,
			clk->mask, clk->cnt, clk->state);
#endif
...
static void init_pll_by_mux(int muxid, int8_t *mux_init,
		int8_t *pll_init, int clkstate, int sysstate)
{
	struct clkmux *mux;
...
static void init_mux_pll_by_clk(struct cg_clk *clk,
		int8_t *mux_init, int8_t *pll_init)
{
	if (!clk)
		return;
...
static void init_mux_pll_by_subsys(struct subsys *sys, int8_t *mux_init,
		int8_t *pll_init)
{
	if (!sys)
		return;
...
static void dump_pll_info1(struct seq_file *s, int clkidx)
{
	if (clkidx < NR_PLLS) {
		...
		seq_printf(s, "[%d] %7s: %4d.%03d MHz: %3s (%2d)\n",
				clkidx, pll->name, freq / 1000, freq % 1000,
				pll->state ? "ON" : "off", pll->cnt);
	}
}
...
static int pll_test_show(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "\n********** pll_test help **********\n");
	seq_printf(s, "enable  pll: echo enable  id [mod_name]"
			"> /proc/clkmgr/pll_test\n");
	seq_printf(s, "disable pll: echo disable id [mod_name]"
			"> /proc/clkmgr/pll_test\n");
#if CLKMGR_8127
	seq_printf(s, "set freq   : echo setfreq id freq_khz"
			"> /proc/clkmgr/pll_test\n");
#endif /* CLKMGR_8127 */
		...
		size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%s %d %u", cmd, &id, &freq) == 3) {
		if (!strcmp(cmd, "setfreq")) {
			...
			if (r != 0)
				clk_info("pll_set_freq(): 0x%08x, id: %d, freq:"
						"%u\n", r, id, freq);
		}
...
static int pll_fsel_show(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "\n********** pll_fsel help **********\n");
	seq_printf(s, "adjust pll frequency:  echo id freq"
...
static int subsys_test_show(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "\n********** subsys_test help **********\n");
	seq_printf(s, "enable subsys:  echo enable id"
			"> /proc/clkmgr/subsys_test\n");
	seq_printf(s, "disable subsys: echo disable id [force_off]"
			"> /proc/clkmgr/subsys_test\n");
...
static int subsys_stat_show(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < NR_SYSS; i++) {
		...
		list_for_each(pos, &sys->head) {
			node = list_entry(pos, struct stat_node, link);
			seq_printf(s, "\t(%s,%u,%u)", node->name, node->cnt_on,
					node->cnt_off);
		}
		seq_printf(s, "\n");
	}
...
static int mux_test_show(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "\n********** mux_test help *********\n");
	seq_printf(s, "clkmux     : echo clkmux  mux_id val [mod_name]"
			"> /proc/clkmgr/mux_test\n");
	seq_printf(s, "enable  mux: echo enable  mux_id [mod_name]"
			"> /proc/clkmgr/mux_test\n");
	seq_printf(s, "disable mux: echo disable mux_id [mod_name]"
			"> /proc/clkmgr/mux_test\n");
		...
		int count, int *eof, void *data)
{
	...
	p += sprintf(p, "\n********** clk_test help **********\n");
	p += sprintf(p, "enable  clk: echo enable  id [mod_name]"
			"> /proc/clkmgr/clk_test\n");
	p += sprintf(p, "disable clk: echo disable id [mod_name]"
			"> /proc/clkmgr/clk_test\n");
...
static uint32_t measure_stable_fmeter_freq(enum FMETER_TYPE type, int k1, int clk)
{
	...
	while (maxfreq > 0 && ABS_DIFF(freq, last_freq) * 100 / maxfreq
			> diff_threshold) {
		last_freq = freq;
...
static int clk_force_on_show(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "\n********** clk_force_on help **********\n");
	seq_printf(s, "set clk force on: echo set id"
			"> /proc/clkmgr/clk_force_on\n");
	seq_printf(s, "clr clk force on: echo clr id"
			"> /proc/clkmgr/clk_force_on\n");
...
{
	clk_info("[%s]: Dump Top MUX register, CLK_CFG_0=0x%x, CLK_CFG_1=0x%x,"
			"CLK_CFG_2=0x%x, CLK_CFG_3=0x%x\n", __func__,
...
int CLK_Monitor_0(enum ckmon_sel ckmon, enum monitor_clk_sel_0 sel, int div)
{
	...
	clk_info("CLK_Monitor_0 Reg: CLK26CALI_0=0x%08x, CLK_CFG_8=0x%08x,"
			"CLK_MISC_CFG_1=0x%08x\n",
