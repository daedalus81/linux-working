...
void eemcs_memory_reserve(void){
	...
	if( (ext_md_usage_case&MD5_EN)== MD5_EN) { //Only MD1 enabled
		...
		ext_md_mem_addr[MD_SYS5-MD_EXT1] =
			(u32) arm_memblock_steal(ext_md_size_list[MD_SYS5-MD_EXT1],
			 	 SZ_32M);
	...
	printk("[EEMCS/PLAT] (0)MemStart(0x%08X):MemSize(0x%08X)\n",
			ext_md_mem_addr[MD_SYS5-MD_EXT1],
			ext_md_size_list[MD_SYS5-MD_EXT1]);
...
void get_ext_md_post_fix(int md_id, char buf[], char buf_ex[])
{
	...
	if (md_id < MD_SYS5) {
		printk("[EEMCS/PLAT] [Error]get_ext_md_post_fix: invalid"
				"md_id=%d\n", md_id);
		return;
	}
...
unsigned int get_ext_modem_support(int md_id){
	...
	switch(md_id) {
		case MD_SYS5:
			...
			break;
		default:
			printk("[EEMCS/PLAT] [Error]get_ext_modem_support:"
					"invalid md_id=%d\n", md_id);
			break;
	}
	return feature_val;
	...
	switch(md_id) {
		case MD_SYS5:
			if (md_type >= modem_lwg &&  md_type <= modem_ltg)
				...
				printk("[EEMCS/PLAT] [Error]set_modem_support"
						"fail(md:%d, md_type:%d)\n",
						md_id, md_type);
				return -1;
			}
			break;
		default:
			printk("[EEMCS/PLAT] [Error]set_modem_support: invalid"
					"md_id=%d\n", md_id);
			return -1;
			break;
	}
...
{
	sprintf(ver, "MT%04x_S%02x", get_chip_code(),
			(get_chip_sw_ver_code()&0xFF));
}
...
unsigned int get_ext_md_mem_start_addr(int md_id){
	...
	switch(md_id) {
		case MD_SYS5:
			...
			break;
		default:
			printk("[EEMCS/PLAT] [Error]get_ext_md_mem_start_addr:"
					"invalid md_id=%d\n", md_id);
			break;
	}
...
unsigned int get_ext_md_mem_size(int md_id){
	...
	switch(md_id) {
		case MD_SYS5:
			...
			break;
		default:
			printk("[EEMCS/PLAT] [Error]get_ext_md_mem_size: invalid"
					"md_id=%d\n", md_id);
			break;
	}
...
int clear_md_region_protection(int md_id)
{
	...
	switch(md_id) {
		...
		default:
			printk("[EEMCS/PLAT] [Error]clear_md_region_protection:"
					"invalid md_id=%d\n", md_id+1);
			return -1;
	}
...
/*                                                                         */
/***************************************************************************/
int eemcs_register_sys_msg_notify_func(int md_id, int (*func)(int,
			unsigned int, unsigned int))
{
	...
	if( ex_md_id >= MAX_EXT_MD_NUM ) {
		printk("[EEMCS/PLAT] [Error]register_sys_msg_notify_func:"
				"invalid md id(%d)\n", md_id+1);
		return E_PARAM;
	}
	...
	if(eemcs_sys_msg_notify_func[ex_md_id] == NULL)
		...
		printk("[EEMCS/PLAT] [Error]eemcs_sys_msg_notify_func: func"
				"registered!\n", md_id+1);
...
int eemcs_notify_md_by_sys_msg(int md_id, unsigned int msg, unsigned int data)
{
	...
	if(ext_md_id >= MAX_EXT_MD_NUM) {
		printk("[EEMCS/PLAT] [Error]notify_md_by_sys_msg: invalid md"
				"id(%d)\n", md_id+1);
		return E_PARAM;
	}
	...
	if(func != NULL) {
		...
		ret = E_NO_EXIST;
		printk("[EEMCS/PLAT] [Error]notify_md_by_sys_msg: func not"
				"register!\n", md_id+1);
	}
...
int eemcs_register_ccci_sys_call_back(int md_id, unsigned int id,
		eemcs_sys_cb_func_t func)
{
	...
	if( ext_md_id >= MAX_EXT_MD_NUM ) {
		printk("[EEMCS/PLAT] [Error]register_sys_call_back: invalid md"
				"id(%d)\n", md_id+1);
		return E_PARAM;
	}
...
void eemcs_exec_ccci_sys_call_back(int md_id, int cb_id, int data)
{
	...
	id = cb_id & 0xFF;
	if(id >= MAX_KERN_API) {
		printk("[EEMCS/PLAT] [Error]exec_sys_cb: invalid func"
				"id(0x%x)\n", cb_id);
		return;
	}
	...
	if(func != NULL)
		...
		printk("[EEMCS/PLAT] [Error]exec_sys_cb: func id(0x%x) not"
				"register!\n", cb_id);
...
/***************************************************************************/
int eemcs_register_ccci_kern_func_by_md_id(int md_id, unsigned int id,
		eemcs_kern_cb_func_t func)
{
	...
	if((id >= MAX_KERN_API) || (func == NULL) || (md_id >= MAX_EXT_MD_NUM)) {
		printk("[EEMCS/PLAT] [Error]register_kern_func: md_id:%d,"
				"func_id:%d!\n", md_id+1, id);
		return E_PARAM;
	}
	...
	if(info_ptr->func == NULL) {
		...
		printk("[EEMCS/PLAT] [Error]register_kern_func: func(%d)"
				"registered!\n", md_id+1, id);
...
int eemcs_exec_ccci_kern_func_by_md_id(int md_id, unsigned int id,
		char *buf, unsigned int len)
{
	...
	if(md_id >= MAX_EXT_MD_NUM) {
		printk("[EEMCS/PLAT] [Error]exec kern func: invalid md"
				"id(%d)\n", md_id+1);
		return E_PARAM;
	}

	if(id >= MAX_KERN_API) {
		printk("[EEMCS/PLAT] [Error]exec kern func: invalid func"
				"id(%d)\n", md_id, id);
		return E_PARAM;
	}
	...
	if(func != NULL) {
		...
		if(eemcs_kern_func_err_num[md_id][id] < SHOW_WARNING_NUM) {
			...
			printk("[EEMCS/PLAT] [Error]exec kern func: func%d not"
					"register\n", md_id+1, id);
		}
	}
...
void eemcs_register_suspend_notify(int md_id, unsigned int id, void (*func)(int))
{
	if((id >= MAX_SLEEP_API) || (func == NULL) || (md_id >= MAX_EXT_MD_NUM))
		printk("[EEMCS/PLAT] [Error]register_suspend_notify: invalid"
				"para(md:%d, cmd:%d)\n", md_id, id);
...
{
	if((id >= MAX_SLEEP_API) || (func == NULL) || (md_id >= MAX_EXT_MD_NUM))
		printk("[EEMCS/PLAT] [Error]register_resume_notify: invalid"
				"para(md:%d, cmd:%d)\n", md_id, id);
...
static int __init eemcs_helper_init(void)
{
	...
	if (ret) {
		printk("[EEMCS/PLAT] [Err]eemcs_helper_device register fail: %d\n",
				ret);
		return ret;
	}
	...
	if (ret) {
		printk("[EEMCS/PLAT] [Err]eemcs_helper_driver register fail: %d\n",
				ret);
		return ret;
	}
