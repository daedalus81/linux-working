...
//this buffer is allocated for
//MODE_MANUAL_USER & MODE_MANUAL_KERNEL only
struct mt_mon_log *mt_mon_log_buff;
...
static inline void set_cpumask(unsigned int cpu,
		unsigned int index, volatile unsigned long *p)
{
...
static inline void clear_bitmap(unsigned int bit,
...
static inline int get_cpumask(unsigned int index,
		volatile unsigned long *p)
{
	unsigned int res;
...
static unsigned int mt65xx_mon_log(void* log_buff)
{
	...
	/* In MODE_SCHED_SWITCH, we need to record the current CPU number to get
	 * the Context Switch CPU number.
	 */
	if( register_mode == MODE_MANUAL_USER
			|| register_mode == MODE_MANUAL_KERNEL) {
	...
	if(mon_buff) {
		...
		for_each_present_cpu(cpu){
			...
			mon_buff->cpu_cyc[cpu] =
				pmu_data->cnt_val[cpu][ARMV7_CYCLE_COUNTER];
		}
		...
		struct trace_iterator *iter)
{
	...
	if(mon_mode_evt != MODE_SCHED_SWITCH){
		...
		{
			trace_seq_printf(s,
					" cpu%d_cyc = %d, cpu%d_cnt0 = %d,"
					"cpu%d_cnt1 = %d, cpu%d_cnt2 = %d,"
					"cpu%d_cnt3 = %d, ",
					cpu,
	...
	trace_seq_printf(s,
			"DRAMC_PageHit = %d, DRAMC_PageMiss = %d,"
			"DRAMC_Interbank = %d, DRAMC_Idle = %d\n",
...
static ssize_t bm_master_evt_store(struct device_driver *driver,
...
static ssize_t bm_rw_type_evt_store(struct device_driver *driver,
		const char *buf, size_t count)
{
...
static ssize_t mon_mode_evt_store(struct device_driver *driver, const char *buf,
		size_t count)
{
	MonitorMode mon_mode_evt;
...
static ssize_t mon_period_evt_store(struct device_driver *driver,
		const char *buf, size_t count)
{
...
static ssize_t mon_manual_evt_show(struct device_driver *driver, char *buf)
{
	...
	if(mon_manual_evt == 1)
		return snprintf(buf, PAGE_SIZE,
				"Manual Monitor is Started (manual mode)\n");

	if(mon_manual_evt == 0)
		return snprintf(buf, PAGE_SIZE,
				"Manual Monitor is Stopped (manual mode)\n");

	return 0;
}

static ssize_t mon_manual_evt_store(struct device_driver *driver,
		const char *buf, size_t count)
{
...
static ssize_t cpu_pmu_evt_store(struct device_driver *driver,
		const char *buf, size_t count)
{
	char *p = (char *)buf;
	char *token[8];
	...
	for(i = 0; i < NUMBER_OF_EVENT; i++)
		p_pmu->perf_cfg.event_cfg[i] =
			simple_strtoul(token[i], &token[i], 16);
...
static ssize_t emi_dcm_ctrl_store(struct device_driver *driver,
		const char *buf, size_t count)
{
...
static int __init mt_mon_mod_init(void)
{
	...
	BM_SetMonitorCounter(1, BM_MASTER_MM,
			BM_TRANS_TYPE_4BEAT
			| BM_TRANS_TYPE_8Byte
		...
		if (!mt_mon_log_buff) {
			mt_mon_log_buff =
				vmalloc(sizeof(struct mt_mon_log)
						* MON_LOG_BUFF_LEN);
			if (!mt_mon_log_buff) {
				printk(KERN_WARNING "fail to allocate the buffer"
						"for the monitor log\n");
