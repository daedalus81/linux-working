...
int mt_cpu_ss_thread_handler(void *unused)
{
	...
	do {
		...
		if (!flag) {
			mt65xx_reg_sync_writel((readl(TOP_CKMUXSEL) & 0x0ff3),
					TOP_CKMUXSEL);
			flag = 1;
		} else {
			mt65xx_reg_sync_writel((readl(TOP_CKMUXSEL) | 0x0004),
					TOP_CKMUXSEL);
			flag = 0;
		}
		...
		if (mt_cpu_ss_debug_mode)
			printk("[%s]: TOP_CKMUXSEL = 0x%x\n",
					__FUNCTION__, readl(TOP_CKMUXSEL));
...
static ssize_t cpu_ss_mode_write(struct file *file,
...
static ssize_t cpu_ss_period_write(struct file *file,
		const char *buffer, size_t count, loff_t *data)
{
	int len = 0, s = 0, ns = 0;
	char desc[32];
...
static ssize_t cpu_ss_period_mode_write(struct file *file,
		const char *buffer, size_t count, loff_t *data)
{
	int len = 0;
	...
	if (sscanf(desc, "%s", mode) == 1) {
		if (!strcmp(mode, "enable")) {
			...
			mt_cpu_ss_thread = kthread_run(mt_cpu_ss_thread_handler,
					0, "cpu speed switch");
			...
			mt65xx_reg_sync_writel((readl(TOP_CKMUXSEL) | 0x0004), TOP_CKMUXSEL);
...
static int __init mt_cpu_ss_init(void)
{
	...
	if (!mt_cpu_ss_dir)
		...
		mt_entry = proc_create("cpu_ss_debug_mode",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_cpu_ss_dir,
				&cpu_ss_debug_mode_fops);
		if (!mt_entry)
			printk("[mtktscpu_init]: create "
					"/proc/cpu_ss/cpu_ss_debug_mode failed\n");

		mt_entry = proc_create("cpu_ss_period_mode",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_cpu_ss_dir,
				&cpu_ss_period_mode_fops);
		if (!mt_entry)
			printk("[mtktscpu_init]: create "
					"/proc/cpu_ss/cpu_ss_period_mode failed\n");

		mt_entry = proc_create("cpu_ss_period",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_cpu_ss_dir, &cpu_ss_period_fops);
		if (!mt_entry)
			printk("[mtktscpu_init]: create "
					"/proc/cpu_ss/cpu_ss_period failed\n");

		mt_entry = proc_create("cpu_ss_mode",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_cpu_ss_dir, &cpu_ss_mode_fops);
		if (!mt_entry)
			printk("[mtktscpu_init]: create "
					"/proc/cpu_ss/cpu_ss_mode failed\n");
	}
	return 0;
}
