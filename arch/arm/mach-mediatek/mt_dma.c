...
int mt_dump_gdma(int channel)
{
	...
	for (i = 0; i < 15; i++) {
		printk("addr:0x%p, value:%x\n",
				...
				readl(DMA_BASE_CH(channel) + i * 4));
	}
	// GDMA DEBUG Status reg is @ channel + 0xD0
	printk("addr:0x%p, value:%x\n", DMA_BASE_CH(channel) + 0xD0,
			readl(DMA_BASE_CH(channel) + 0xD0));
...
int mt_hard_reset_gdma(int channel)
{
	...
	mt_reg_sync_writel(DMA_HARD_RST_BIT,
			DMA_RESET(DMA_BASE_CH(channel)));
	mt_reg_sync_writel(DMA_HARD_RST_CLR_BIT,
			DMA_RESET(DMA_BASE_CH(channel)));
...
static irqreturn_t gdma1_irq_handler(int irq, void *dev_id)
{
	...
	if (glbsta & DMA_GLBSTA_IT(G_DMA_1)){
		...
		mt_reg_sync_writel(DMA_INT_FLAG_CLR_BIT,
				DMA_INT_FLAG(DMA_BASE_CH(G_DMA_1)));
#if(DMA_DEBUG == 1)
...
static irqreturn_t gdma2_irq_handler(int irq, void *dev_id)
{
	...
	if (glbsta & DMA_GLBSTA_IT(G_DMA_2)){
		...
		mt_reg_sync_writel(DMA_INT_FLAG_CLR_BIT,
				DMA_INT_FLAG(DMA_BASE_CH(G_DMA_2)));
...
void APDMA_test_transfer(int testcase)
{
	...
	dma_dst_array_v = dma_alloc_coherent(NULL,
			TEST_LEN, &dma_dst_array_p, GFP_KERNEL );
	dma_src_array_v = dma_alloc_coherent(NULL,
			TEST_LEN, &dma_src_array_p, GFP_KERNEL );
...
static int __init mt_init_dma(void)
{
	...
	if (request_irq(MT_GDMA1_IRQ_ID, gdma1_irq_handler,
				IRQF_TRIGGER_LOW, "GDMA1",  NULL))
		printk(KERN_ERR"GDMA1 IRQ LINE NOT AVAILABLE!!\n");

	if (request_irq(MT_GDMA2_IRQ_ID, gdma2_irq_handler,
				IRQF_TRIGGER_LOW, "GDMA2",  NULL))
