...
/* irq_total_secondary_cpus is initialized in
 * smp_init_cpus() of platsmp.c
 * */
...
static int mt_irq_set_type(struct irq_data *data, unsigned int flow_type)
{
	...
	if (irq > (NR_GIC_SGI + NR_GIC_PPI)) {
		...
		while (__check_irq_type[i].num >= 0) {
			int num = __check_irq_type[i].num;
			int sens = __check_irq_type[i].sensitivity;
			int pol = __check_irq_type[i].polarity;

			if (num == irq) {
				if (flow_type & (IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING)) {
					if (sens != MT_EDGE_SENSITIVE)
						pr_err("error: level-sensitive irq %d is set as edge-trigger\n", irq);
				} else if (flow_type & (IRQF_TRIGGER_HIGH | IRQF_TRIGGER_LOW)) {
					if (sens != MT_LEVEL_SENSITIVE)
						pr_err("error: edge-trigger irq %d is set as level-sensitive\n", irq);
				}

				if (flow_type & (IRQF_TRIGGER_RISING | IRQF_TRIGGER_HIGH)) {
					if (pol != 1)
						pr_err("error: active-low irq %d is set as active-high\n", irq);
				} else if (flow_type & (IRQF_TRIGGER_FALLING | IRQF_TRIGGER_LOW)) {
					if (pol != 0)
						pr_err("error: active-high irq %d is set as active-low\n", irq);
				}
...
void mt_irq_dump_status(int irq)
{
	...
	printk("[INT] irq:%d,enable:%x,active:%x,pending:%x,pri:%x,"
			"grp:%x,target:%x,sens:%x,pol:%x\n",
