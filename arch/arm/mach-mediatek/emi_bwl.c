...
static ssize_t con_sce_store(struct device_driver *driver, const char *buf, size_t count)
{
	...
	for (i = 0; i < NR_CON_SCE; i++) {
		if (!strncmp(buf, con_sce_str[i], strlen(con_sce_str[i]))) {
			if (!strncmp(buf + strlen(con_sce_str[i]) + 1, EN_CON_SCE_STR,
 ...
 * Return the number of write bytes.
 */
static ssize_t finetune_mm_store(struct device_driver *driver,
...
static int __init emi_bwl_mod_init(void)
{
	...
	if (ret)
		printk(ANDROID_LOG_ERROR, "EMI/BWL",
				"fail to set EMI bandwidth limiter\n");
	...
	if (ret)
		printk(ANDROID_LOG_ERROR, "EMI/BWL",
				"fail to register DRAM_TYPE driver\n");

	ret = driver_create_file(&ddr_type.driver,
			&driver_attr_ddr_type);
	if (ret)
		printk(ANDROID_LOG_ERROR, "EMI/BWL",
				"fail to create DRAM_TYPE sysfs file\n");
