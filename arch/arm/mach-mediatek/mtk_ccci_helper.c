...
#define TDD_HALT_STATUS_ADDR(base)				((volatile unsigned int*)((base) + 0x0002))
	...
	mt65xx_reg_sync_writel(0xFFFFFFFF,
			MODEM2G_TOPSM_SM_REQ_MASK(modem2g_topsm_base));
	mt65xx_reg_sync_writel(0xFFFFFFFF,
			MODEM2G_TOPSM_RM_PLL_MASK0(modem2g_topsm_base));
	mt65xx_reg_sync_writel(0xFFFFFFFF,
			MODEM2G_TOPSM_RM_PLL_MASK1(modem2g_topsm_base));
...
static void cal_md_mem_usage(void)
{
	...
	if(md1_en && (md_share_mem_size[MD_SYS1]>0) && (md_resv_mem_size[MD_SYS1]>0)) {
		...
		printk("[ccci/ctl] (0)[Error]DFO Setting for md1 wrong:"
				"<%d:0x%08X:0x%08X>\n", md1_en,
				md_resv_mem_size[MD_SYS1],
				md_share_mem_size[MD_SYS1]);
		md_share_mem_size[MD_SYS1] = MD1_SMEM_SIZE;
		md_resv_mem_size[MD_SYS1] = MD1_MEM_SIZE;
	} else {
		// Has conflict
		printk("[ccci/ctl] (0)[Error]DFO Setting for md1 conflict:"
				"<%d:0x%08X:0x%08X>\n", md1_en,
				md_resv_mem_size[MD_SYS1],
				md_share_mem_size[MD_SYS1]);
		md1_en = 0;
...
void ccci_md_mem_reserve(void)
{
	...
	if(ptr) {
		...
		printk("[ccci/ctl] (0)md mem reserve successfully,"
				"ptr=%p,size=%d\n",
...
static void collect_md_setting(void)
{
  ...
  if( (md_usage_case&MD1_EN)== MD1_EN) { //Only MD1 enabled
		...
		md_resv_smem_addr[MD_SYS1] = (unsigned int)(addr[0] +
				md_resv_mem_size[MD_SYS1]);
	...
	if ( (md_resv_smem_addr[MD_SYS1]&(2*1024*1024 - 1)) != 0 )
		printk("[ccci/ctl] (0) md1 share memory addr %08x is not 2M"
...
unsigned int set_modem_support(int md_id, int md_type)
{
	...
	case MD_SYS1:
		if (md_type >= modem_2g && md_type <= modem_tg)
			...
			printk("[ccci/ctl] error: set_modem_support fail(md:%d,"
				"md_type:%d)!\n", md_id+1, md_type);
		return 0;
...
int get_eint_attr(char *name, unsigned int name_len,
		unsigned int type, char * result, unsigned int *len)
{
#if defined (FEATURE_GET_MD_EINT_ATTR)
...
*****************************************/
ssize_t mtk_ccci_attr_show(struct kobject *kobj, struct attribute *attr,
		char *buffer);
ssize_t mtk_ccci_attr_store(struct kobject *kobj, struct attribute *attr,
		const char *buffer, size_t size);
ssize_t mtk_ccci_filter_show(struct kobject *kobj, char *page);
ssize_t mtk_ccci_filter_store(struct kobject *kobj, const char *page,
		size_t size);
...
ssize_t mtk_ccci_filter_store(struct kobject *kobj, const char *buffer, size_t size)
{
	...
	for(i=0; i<MAX_FILTER_MEMBER; i++){
		if( strncmp(buffer, cmd_map_table[i].cmd,
		...
		ccci_kern_cb_func_t func)
{
	...
	if((id >= MAX_KERN_API) || (func == NULL) || (md_id >= MAX_MD_NUM)) {
		printk("[ccci/ctl] (0)register kern func fail: md_id:%d,"
				"func_id:%d!\n", md_id+1, id);
		return E_PARAM;
	}
	...
	if(info_ptr->func == NULL) {
		...
		printk("[ccci/ctl] (%d)register kern func fail: func(%d)"
				"registered!\n", md_id+1, id);
...
int exec_ccci_kern_func_by_md_id(int md_id, unsigned int id, char *buf, unsigned int len)
{
	...
	if(id >= MAX_KERN_API) {
		printk("[ccci/ctl] (%d)exec kern func fail: invalid func"
				"id(%d)!\n", md_id, id);
		return E_PARAM;
	}
	...
	if(func != NULL) {
		...
		if(kern_func_err_num[md_id][id] < SHOW_WARNING_NUM) {
			...
			printk("[ccci/ctl] (%d)exec kern func fail: func%d not"
					"register!\n", md_id+1, id);
		}
	}
...
int register_sys_msg_notify_func(int md_id, int (*func)(int, unsigned int, unsigned int))
{
	...
	if( md_id >= MAX_MD_NUM ) {
		printk("[ccci/ctl] (0)register_sys_msg_notify_func fail: invalid"
				"md id(%d)\n", md_id+1);
		return E_PARAM;
	}
	...
	if(ccci_sys_msg_notify_func[md_id] == NULL)
		...
		printk("[ccci/ctl] (%d)ccci_sys_msg_notify_func fail: func"
				"registered!\n", md_id+1);
...
int notify_md_by_sys_msg(int md_id, unsigned int msg, unsigned int data)
{
	...
	if(md_id >= MAX_MD_NUM) {
		printk("[ccci/ctl] (0)notify_md_by_sys_msg: invalid md"
				"id(%d)\n", md_id+1);
		return E_PARAM;
	}
	...
	if(func != NULL)
		...
		ret = E_NO_EXIST;
		printk("[ccci/ctl] (%d)notify_md_by_sys_msg fail: func not"
				"register!\n", md_id+1);
	}
...
int register_ccci_sys_call_back(int md_id, unsigned int id, ccci_sys_cb_func_t func)
{
	...
	if( md_id >= MAX_MD_NUM ) {
		printk("[ccci/ctl] (0)register_sys_call_back fail: invalid md"
				"id(%d)\n", md_id+1);
		return E_PARAM;
	}
	...
	if((id >= 0x100)&&((id-0x100) < MAX_KERN_API))
		...
		printk("[ccci/ctl] (%d)register_sys_call_back fail: invalid func"
				"id(0x%x)\n", md_id+1, id);
		return E_PARAM;
	}
	...
	if(info_ptr->func == NULL) {
		...
		printk("[ccci/ctl] (%d)register_sys_call_back fail: func(0x%x)"
				"registered!\n", md_id+1, id);
...
void exec_ccci_sys_call_back(int md_id, int cb_id, int data)
{
	...
	id = cb_id & 0xFF;
	if(id >= MAX_KERN_API) {
		printk("[ccci/ctl] (%d)exec_sys_cb fail: invalid func"
				"id(0x%x)\n",  md_id+1, cb_id);
		return;
	}
	...
	if(func != NULL)
		...
		printk("[ccci/ctl] (%d)exec_sys_cb fail: func id(0x%x) not"
				"register!\n", md_id+1, cb_id);
}
...
void register_suspend_notify(int md_id, unsigned int id, void (*func)(int))
{
	if((id >= MAX_SLEEP_API) || (func == NULL) || (md_id >= MAX_MD_NUM))
		printk("[ccci/ctl] (0)invalid suspend parameter(md:%d,"
			"cmd:%d)!\n", md_id, id);
...
{
	if((id >= MAX_SLEEP_API) || (func == NULL) || (md_id >= MAX_MD_NUM))
		printk("[ccci/ctl] (0)invalid resume parameter(md:%d,"
			"cmd:%d)!\n", md_id, id);
...
int exec_ccci_kern_func_by_md_id(int md_id, unsigned int id,
