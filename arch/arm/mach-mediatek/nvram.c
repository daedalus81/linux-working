...
/*If index == RT2860, the memory
 * size have to be
 * enlarged since VoIP+AP SoC
 * configuration*/
...
int ra_mtd_write_nm(char *name, loff_t to, size_t len, const u_char *buf)
{
	...
	RANV_PRINT("%s(%d)- name:%s val:%s \n",__func__,__LINE__, name,buf);

	if (!strcmp(name, RALINK_NVRAM_MTDNAME))
		fd = filp_open("/dev/mmcblk0p5", O_WRONLY, 0);
	else
		fd = filp_open("/dev/mmcblk0p6", O_WRONLY, 0);
	...
	RANV_PRINT("%s(%d): pos=0x%x\n",__func__,__LINE__,
			(unsigned int) fd->f_pos);

	retLen = fd->f_op->write(fd, buf, len, &fd->f_pos);

	RANV_PRINT("%s(%d): return=%d\n",__func__,__LINE__, retLen);
...
int ra_mtd_read_nm(char *name, loff_t from, size_t len, u_char *buf)
{
	...
	RANV_PRINT("%s(%d)- name:%s \n",__func__,__LINE__,name);

	if (!strcmp(name, RALINK_NVRAM_MTDNAME))
		fd = filp_open("/dev/mmcblk0p5", O_RDONLY, 0);
	else
		fd = filp_open("/dev/mmcblk0p6", O_RDONLY, 0);
	...
	if (fd->f_pos != from) {
		if (fd->f_op->llseek) {
			RANV_PRINT("%s(%d)\n",__func__,__LINE__);
			if(fd->f_op->llseek(fd, from, 0) != from) {
				printk("[nvram_read] : failed to seek!!\n");
				return -1;
			}
		} else {
...
long ralink_nvram_ioctl(struct file *file, unsigned int req,
		unsigned long arg)
#else
int ralink_nvram_ioctl(struct inode *inode, struct file *file,
...
#endif
{
	...
	switch (req) {
		...
		case RALINK_NVRAM_IOCTL_GETALL:
			...
			index = nvr->index;
#ifdef CONFIG_CONFIG_SHRINK
			len = ENLARGED_DATE_SIZE(index,fb[index].flash_max_len,
					sizeof(fb[index].env.crc));
#else
...
int ra_nvram_init(void)
{
	...
	class_dev = device_create(nvram_class, NULL, nvram_dev, NULL,
			RALINK_NVRAM_DEVNAME);
...
#if defined CONFIG_EXTEND_NVRAM
	if (i >= CONFIG2_NVRAM)
		ra_mtd_read_nm(RALINK_NVRAM2_MTDNAME, from, crcLen,
				(unsigned char *)&fb[i].env.crc);
	else
#endif
		ra_mtd_read_nm(RALINK_NVRAM_MTDNAME, from, crcLen,
				(unsigned char *)&fb[i].env.crc);
...
#if defined CONFIG_EXTEND_NVRAM
	if (i >= CONFIG2_NVRAM)
		ra_mtd_read_nm(RALINK_NVRAM2_MTDNAME, from, len,
				(unsigned char *)fb[i].env.data);
	else
#endif
		ra_mtd_read_nm(RALINK_NVRAM_MTDNAME, from, len,
				(unsigned char *)fb[i].env.data);
...
static void ra_nvram_exit(void)
{
	...
	dev_t nvram_dev = MKDEV(ralink_nvram_major, 0);
...
int nvram_clear(int index)
{
	...
	len = fb[index].flash_max_len - sizeof(fb[index].env.crc);
#ifdef CONFIG_CONFIG_SHRINK
	envDataLen = ENLARGED_DATE_SIZE(index,fb[index].flash_max_len,
			sizeof(fb[index].env.crc));
#else
	envDataLen = len;
#endif
	...
	//calculate crc
	fb[index].env.crc = (unsigned long)nv_crc32(0,
			(unsigned char *)fb[index].env.data, len);
	...
	if (index >= CONFIG2_NVRAM)
		ra_mtd_write_nm(RALINK_NVRAM2_MTDNAME, to, len,
				(unsigned char *)&fb[index].env.crc);
	else
#endif
		ra_mtd_write_nm(RALINK_NVRAM_MTDNAME, to, len,
				(unsigned char *)&fb[index].env.crc);
	...
	if (index >= CONFIG2_NVRAM)
		ra_mtd_write_nm(RALINK_NVRAM2_MTDNAME, to, len,
				(unsigned char *)fb[index].env.data);
	else
#endif
		ra_mtd_write_nm(RALINK_NVRAM_MTDNAME, to, len,
				(unsigned char *)fb[index].env.data);
...
int nvram_commit(int index)
{
	...
	for (i = 0; i < MAX_CACHE_ENTRY; i++) {
		...
		l = strlen(fb[index].cache[i].name) +
			strlen(fb[index].cache[i].value) + 2;

		if (p - fb[index].env.data + 2 >= envDataLen) {
			RANV_ERROR("ENV_BLK_SIZE 0x%x is not enough!",
					ENV_BLK_SIZE);
			up(&nvram_sem);
			return -1;
		}

		snprintf(p, l, "%s=%s", fb[index].cache[i].name,
	...
	if(!ret){
		...
		if (-1 == idx) {
			...
			if (idx == MAX_CACHE_ENTRY) {
				RANV_ERROR("run out of env cache, please"
						"increase MAX_CACHE_ENTRY\n");
				up(&nvram_sem);
				return -1;
			}
		...
		RANV_PRINT("[%s: ORI] name:%s val:%s \n",
				__func__,
				fb[index].cache[idx].name,
				fb[index].cache[idx].value);
#ifdef CONFIG_CONFIG_SHRINK
	}
#endif
	fb[index].dirty = 1;
