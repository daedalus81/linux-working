...
/*
 * armv7_pmnc_counter_select_counter: select monitor counter and return the
 * selected monitor counter register index
...
static void smp_pmu_read_counter(void)
{
	...
	if(armv7pmu.multicore) {
		for(i = 0; i < NUMBER_OF_CPU; i++)
			mtk_smp_call_function_single(i,
					armv7pmu_read_all_counter, NULL, 1);
	} else
#endif
