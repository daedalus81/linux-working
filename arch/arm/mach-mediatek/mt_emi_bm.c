...
int BM_GetTransTypeCount(const unsigned int counter_num)
{
	return (counter_num < 1 || counter_num > BM_COUNTER_MAX)
		? BM_ERR_WRONG_REQ
		: readl(IOMEM(EMI_TTYPE1 + (counter_num - 1) * 8));
}
		...
		const unsigned int master, const unsigned int trans_type)
{
	...
	if (counter_num == 1) {
		...
		value = (readl(IOMEM(addr)) & ~(iMask << 16))
			| ((trans_type & 0xFF) << 24) | ((master & 0x7F) << 16);
	} else {
		addr = (counter_num <= 3)
...
int BM_SetIDSelect(const unsigned int counter_num, const unsigned int id,
...
int BM_SetLatencyCounter(void)
{
	...
	//emi_ttype1 -emi_ttype7 change as total latencies for m0 -- m6, and
	//emi_ttype9 -emi_ttype15 change as total transaction counts for m0--m6
	value |= (0b10 << 24);
