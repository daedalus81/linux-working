...
static inline unsigned int gpt_get_and_ack_irq(void)
{
	unsigned int id;
	unsigned int mask;
	unsigned int status = readl(GPT_IRQSTA);

	for (id = GPT1; id < NR_GPTS; id++) {
		mask = 0x1 << id;
		if (status & mask) {
			writel(GPT_IRQACK, mask);
			break;
		}
	}
	return id;
...
{
	writel(GPT_IRQEN, 0x1 << (dev->id));
}

static void __gpt_disable_irq(struct gpt_device *dev)
{
	u32 tmp = readl(GPT_IRQEN);
	tmp &= ~(0x1 << (dev->id));
	writel(GPT_IRQEN, tmp);
	//DRV_ClrReg32(GPT_IRQEN, 0x1 << (dev->id));
}

static void __gpt_ack_irq(struct gpt_device *dev)
{
	writel(GPT_IRQACK, 0x1 << (dev->id));
}

static void __gpt_reset(struct gpt_device *dev)
{
	writel(dev->base_addr + GPT_CON, 0x0);
	__gpt_disable_irq(dev);
	__gpt_ack_irq(dev);
	writel(dev->base_addr + GPT_CLK, 0x0);
	writel(dev->base_addr + GPT_CON, 0x2);
	writel(dev->base_addr + GPT_CMP, 0x0);
	if (dev->features & GPT_FEAT_64_BIT) {
		writel(dev->base_addr + GPT_CMPH, 0);
	}
}

static void __gpt_clrcnt(struct gpt_device *dev)
{
	writel(dev->base_addr + GPT_CON, GPT_CON_CLRCNT);
	while (readl(dev->base_addr + GPT_CNT)) {
	}
}

static void __gpt_start(struct gpt_device *dev)
{
	writel(dev->base_addr + GPT_CON, GPT_CON_ENABLE);
}

static void __gpt_start_from_zero(struct gpt_device *dev)
{
	//writel(dev->base_addr + GPT_CON, GPT_CON_ENABLE | GPT_CON_CLRCNT);
	__gpt_clrcnt(dev);
	__gpt_start(dev);
}
...
{
	return !!(readl(dev->base_addr + GPT_CON) & GPT_CON_ENABLE);
}

static void __gpt_stop(struct gpt_device *dev)
{
	u32 tmp = readl(dev->base_addr + GPT_CON);
	tmp &= ~(0x1 << (dev->id));
	writel(dev->base_addr + GPT_CON, tmp);
	//DRV_ClrReg32(dev->base_addr + GPT_CON, GPT_CON_ENABLE);
}

static void __gpt_set_mode(struct gpt_device *dev, unsigned int mode)
{
	unsigned int ctl = readl(dev->base_addr + GPT_CON);
	mode <<= GPT_OPMODE_OFFSET;

	ctl &= ~GPT_CON_OPMODE;
	ctl |= mode;

	writel(dev->base_addr + GPT_CON, ctl);
		...
		unsigned int clkdiv)
{
	...
	writel(dev->base_addr + GPT_CLK, clk);
...
static void __gpt_set_cmp(struct gpt_device *dev, unsigned int cmpl,
		unsigned int cmph)
{
	writel(dev->base_addr + GPT_CMP, cmpl);
	dev->cmp[0] = cmpl;

	if (dev->features & GPT_FEAT_64_BIT) {
		writel(dev->base_addr + GPT_CMPH, cmph);
		dev->cmp[1] = cmpl;
	}
}
...
{
	*ptr = readl(dev->base_addr + GPT_CMP);
	if (dev->features & GPT_FEAT_64_BIT) {
		*(++ptr) = readl(dev->base_addr + GPT_CMPH);
	}
}

static void __gpt_get_cnt(struct gpt_device *dev, unsigned int *ptr)
{
	*ptr = readl(dev->base_addr + GPT_CNT);
	if (dev->features & GPT_FEAT_64_BIT) {
		*(++ptr) = readl(dev->base_addr + GPT_CNTH);
	}
}
...
static void syscnt_assist_handler(unsigned long data)
{
	...
	do {
		cnt++;
		cnth = readl(syscnt_dev->base_addr + GPT_CNTH);
		if ((cnt / CHECK_WARNING_TIMERS) && !(cnt % CHECK_WARNING_TIMERS))
			printk("[%s]WARNING: fail to sync GPT_CNTH!!"
					"assist(0x%08x),syscnt(0x%08x,0x%08x),"
					"cnth(0x%08x),loop(0x%08x),cnt(%d)\n",
					__func__, assist_cnt, syscnt_cnt[0],
					syscnt_cnt[1], cnth, loop, cnt);
	} while (cnth != loop);
	...
	printk("[%s]syscnt assist IRQ!! assist(0x%08x),syscnt(0x%08x,0x%08x),"
			...
			__func__, assist_cnt, syscnt_cnt[0],
			syscnt_cnt[1], cnth, pct_hi, loop, cnt);
}
...
static void syscnt_assist_resume(void)
{
	...
	printk("[%s]assist(0x%08x, 0x%08x),syscnt(0x%08x,0x%08x),loop(%u->%u)\n",
			__func__, assist_cnt1, assist_cnt2,
			syscnt_cnt[0], syscnt_cnt[1], old_loop, loop);
}
...
MODULE_DESCRIPTION("MTK GPT Driver v0.1");
MODULE_LICENSE("GPL");
