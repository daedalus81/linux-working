...
static unsigned int mt_cpufreq_limit_max_freq_by_early_suspend(void)
{
	...
	printk("Power/DVFS:mt_cpufreq limited max freq by early suspend %d\n",
			DVFS_F2);
...
unsigned int mt_cpufreq_voltage_set_by_ptpod(unsigned int pmic_volt[],
		unsigned int array_size)
{
	int i;//, idx;
	unsigned long flags;
	unsigned int PMIC_WRAP_DVFS_WDATA_array[8] = {
		PMIC_WRAP_DVFS_WDATA0,
		PMIC_WRAP_DVFS_WDATA1,
		PMIC_WRAP_DVFS_WDATA2,
		PMIC_WRAP_DVFS_WDATA3,
		PMIC_WRAP_DVFS_WDATA4,
		PMIC_WRAP_DVFS_WDATA5,
		PMIC_WRAP_DVFS_WDATA6,
		PMIC_WRAP_DVFS_WDATA7
	};

	if(array_size > (sizeof(mt_cpufreq_pmic_volt)/4))
		dprintk("mt_cpufreq_voltage_set_by_ptpod:"
				"ERROR!array_size is invalide, array_size = %d\n",
				array_size);
	...
	dprintk("mt_cpufreq_voltage_set_by_ptpod:"
			"Set voltage directly by PTP-OD request!\n");
...
unsigned int mt_cpufreq_max_frequency_by_DVS(unsigned int num)
{
	...
	if(num == voltage_change_num) {
		printk(KERN_INFO "Power/DVFS:PTPOD0:num = %d, frequency= %d\n",
	...
	for (i = 1; i < mt_cpu_freqs_num; i++) {
		...
		if(num == voltage_change_num) {
			printk(KERN_INFO "Power/DVFS:PTPOD1:num = %d, frequency= %d\n",
					num, mt_cpu_freqs[i].cpufreq_khz);
			return mt_cpu_freqs[i].cpufreq_khz;
		}
	}

	printk(KERN_INFO "Power/DVFS: PTPOD2:num = %d, NOT found! return 0!\n",
			num);
	return 0;
}
...
static void mt_cpufreq_power_calculation(int index, int ncpu)
{
	...
	volt_square_ratio = (((mt_cpu_freqs[index].cpufreq_volt * 100) / 1150)
	...
	multi = ((mt_cpu_freqs[index].cpufreq_volt * 100) / 1150)
	...
	mt_cpu_power[index * possiblecpu + ncpu].cpufreq_khz =
		mt_cpu_freqs[index].cpufreq_khz;
	mt_cpu_power[index * possiblecpu + ncpu].cpufreq_power =
		p_dynamic + p_leakage;

	printk(KERN_INFO "Power/DVFS:mt_cpu_power[%d]: cpufreq_ncpu = %d,"
			"cpufreq_khz = %d, cpufreq_power = %d\n",
...
static void mt_setup_power_table(int num)
{
	...
	dprintk("P_CA7_D_1_CORE = %d, P_CA7_D_2_CORE = %d,"
			"P_CA7_D_3_CORE = %d, P_CA7_D_4_CORE = %d\n",
			P_CA7_D_1_CORE, P_CA7_D_2_CORE,
			P_CA7_D_3_CORE, P_CA7_D_4_CORE);
	...
	memset( (void *)mt_cpufreq_power_efficiency, 0,
			sizeof(unsigned int)*20*10 );

	mt_cpu_power = kzalloc((num * possiblecpu)
	...
	for (i = 0; i < num; i++) {
		for (j = 0; j < possiblecpu; j++) {
			...
			if(((mt_cpu_freqs_table[i].frequency == DVFS_F0)
						&& (ncpu == 3))
					|| ((mt_cpu_freqs_table[i].frequency == DVFS_F0)
						&& (ncpu == 2))) {
 	...
 	/* Need to check, if condition core num change.*/
	g_cpu_power_table_num = num * possiblecpu - count;
	...
	for (i = (num * possiblecpu - 1); i > 0; i--) {
		for (j = 1; j <= i; j++) {
			if (mt_cpu_power[j - 1].cpufreq_power
					< mt_cpu_power[j].cpufreq_power) {
				temp_power_info.cpufreq_khz =
					mt_cpu_power[j - 1].cpufreq_khz;
				temp_power_info.cpufreq_ncpu =
					mt_cpu_power[j - 1].cpufreq_ncpu;
				temp_power_info.cpufreq_power =
					mt_cpu_power[j - 1].cpufreq_power;

				mt_cpu_power[j - 1].cpufreq_khz =
					mt_cpu_power[j].cpufreq_khz;
				mt_cpu_power[j - 1].cpufreq_ncpu =
					mt_cpu_power[j].cpufreq_ncpu;
				mt_cpu_power[j - 1].cpufreq_power =
					mt_cpu_power[j].cpufreq_power;
	...
	for (i = 0; i < (num * possiblecpu); i++) {
		dprintk("mt_cpu_power[%d].cpufreq_khz = %d, ",
				i, mt_cpu_power[i].cpufreq_khz);
		dprintk("mt_cpu_power[%d].cpufreq_ncpu = %d, ",
 ...
 ************************************************/
static int mt_setup_freqs_table(struct cpufreq_policy *policy,
		struct mt_cpu_freq_info *freqs, int num)
{
	...
	if (!ret)
		cpufreq_frequency_table_get_attr(mt_cpu_freqs_table,
				policy->cpu);
...
int mt_cpufreq_state_set(int enabled)
{
	if (enabled) {
		if (!mt_cpufreq_pause) {
			printk(KERN_INFO "Power/DVFS:"
					"cpufreq already enabled\n");
			return 0;
		}
		...
		printk(KERN_INFO "Power/DVFS: enable DVFS:"
				"g_dvfs_disable_count = %d\n",
				g_dvfs_disable_count);
		...
		if (g_dvfs_disable_count <= 0)
			...
			printk(KERN_INFO "Power/DVFS:cpufreq is disabled,"
					"cannot enable it\n");
	} else {
		/**************
		 * disable DVFS
		 ***************/
		g_dvfs_disable_count++;
		printk(KERN_INFO "Power/DVFS: disable DVFS:"
				"g_dvfs_disable_count = %d\n",
				g_dvfs_disable_count);

		if (mt_cpufreq_pause) {
			printk(KERN_INFO "Power/DVFS"
					"cpufreq already disabled\n");
			return 0;
		}
...
void mt_cpufreq_disable(unsigned int type, bool disabled)
{
	...
	if(type == 0) {
		...
		if(mt_cpufreq_disabled_by_sdio_autoK == true) {
			...
			printk(KERN_INFO "Power/DVFS:"
					"mt_cpufreq_disable, limited freq. at %d\n",
					DVFS_F2);

			cpufreq_cpu_put(policy);
		}
		printk(KERN_INFO "Power/DVFS: mt_cpufreq_disable"
				"mt_cpufreq_disabled_by_sdio_autoK = %d\n",
...
void mt_cpufreq_volt_set_by_sdio(unsigned int volt)
{
	...
	printk(KERN_INFO "Power/DVFS:"
			"mt_cpufreq_volt_set_by_sdio, reg = %d\n", reg);
	...
	if(mt_cpufreq_disabled_by_sdio_autoK == true) {
		...
		printk(KERN_INFO "Power/DVFS:"
				"mt_cpufreq_volt_set_by_sdio, could not set volt now,"
				"mt_cpufreq_disabled_by_sdio_autoK = %d\n",
				mt_cpufreq_disabled_by_sdio_autoK);
	}
#endif
 ...
 ************************************************************/
static void mt_cpufreq_set(unsigned int freq_old,
		unsigned int freq_new, unsigned int target_volt)
{
	...
	if (freq_new > freq_old) {
		...
		if(((freq_new > FHCTL_CHANGE_FREQ) &&
					(freq_old > FHCTL_CHANGE_FREQ))
				|| ((freq_new < FHCTL_CHANGE_FREQ)
					&& (freq_old < FHCTL_CHANGE_FREQ))) {
 ...
 ***************************************/
static int mt_cpufreq_keep_max_freq(unsigned int freq_old,
...
static int mt_cpufreq_target(struct cpufreq_policy *policy, unsigned int target_freq, unsigned int relation)
{
	...
	if(mt_cpufreq_limit_max_freq_early_suspend == true) {
		freqs.new = DVFS_F2;
		dprintk("mt_cpufreq_limit_max_freq_early_suspend,"
				"freqs.new = %d\n", freqs.new);
	}
	...
	if (freqs.new < g_limited_min_freq) {
		dprintk("cannot switch CPU frequency to %d Mhz"
	...
	if (freqs.old == freqs.new) {
		dprintk("CPU frequency from %d MHz to %d MHz"
				"(skipped) due to same frequency\n",
...
void mt_cpufreq_return_default_DVS_by_ptpod(void)
{
	...
	printk(KERN_INFO"Power/DVFS:"
			"mt_cpufreq return default DVS by ptpod\n");
}
...
void mt_cpufreq_enable_by_ptpod(void)
{
	...
	printk(KERN_INFO "Power/DVFS: mt_cpufreq enabled by ptpod\n");
}
...
unsigned int mt_cpufreq_disable_by_ptpod(void)
{
	...
	printk(KERN_INFO "Power/DVFS:"
			"mt_cpufreq disabled by ptpod, limited freq. at %d\n",
			DVFS_F2);
...
void mt_cpufreq_thermal_protect(unsigned int limited_power)
{
	...
	if (limited_power == 0)	{
		...
		printk(KERN_INFO, "Power/DVFS:thermal limit g_limited_max_freq = %d,"
				"g_limited_max_ncpu = %d\n",
				g_limited_max_freq,
				g_limited_max_ncpu);
	} else {
		while (ncpu) {
		...
		if (!found) {
			printk(KERN_INFO "Power/DVFS:Not found suitable DVFS OPP,"
					"limit to lowest OPP!\n");
			g_limited_max_ncpu =
				mt_cpu_power[g_cpu_power_table_num - 1].cpufreq_ncpu;
			g_limited_max_freq =
				mt_cpu_power[g_cpu_power_table_num - 1].cpufreq_khz;
#if defined(CONFIG_THERMAL_LIMIT_TEST)
			g_limited_max_thermal_power =
				mt_cpu_power[g_cpu_power_table_num - 1].cpufreq_power;
#endif
		}
		...
		if (num_online_cpus() > g_limited_max_ncpu) {
			for (i = num_online_cpus(); i > g_limited_max_ncpu; i--) {
				printk(KERN_INFO "Power/DVFS:turn off CPU%d"
...
static ssize_t mt_cpufreq_state_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(buffer, "%d", &enabled) == 1) {
		if (enabled == 1)
			...
			printk(KERN_INFO"Power/DVFS:bad argument!!"
					"argument should be \"1\" or \"0\"\n");
	} else {
		printk(KERN_INFO "Power/DVFS:bad argument!!"
				"argument should be \"1\" or \"0\"\n");
	}
 ...
 ***********************************/
static ssize_t mt_cpufreq_limited_power_write(struct file *file,
		const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(buffer, "%u", &power) == 1) {
		...
		printk(KERN_INFO "Power/DVFS: bad argument!!"
				"please provide the maximum limited power\n");
 ...
 ***********************************/
static ssize_t mt_cpufreq_limited_load_write(struct file *file,
		const char *buffer, size_t count, loff_t *data)
{
	unsigned int load = 0;
	...
	if (sscanf(buffer, "%u", &load) == 1) {
		...
		printk(KERN_INFO "Power/DVFS:bad argument!!"
				"please provide the limited load\n");
 ...
 ************************/
static ssize_t mt_cpufreq_debug_write(struct file *file,
		const char *buffer, size_t count, loff_t *data)
{
	int debug = 0;
	...
	if (sscanf(buffer, "%d", &debug) == 1) {
		if (debug == 0) {
			...
			printk(KERN_INFO "Power/DVFS:bad argument!!"
					"should be 0 or 1 [0: disable, 1: enable]\n");
	} else
		printk(KERN_INFO "Power/DVFS:bad argument!!"
				"should be 0 or 1 [0: disable, 1: enable]\n");
...
static int mt_cpufreq_pdrv_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "Power/DVFS: mediatek cpufreq initialized\n");
...
static int __init mt_cpufreq_pdrv_init(void)
{
	...
	if (!mt_cpufreq_dir)
		...
		mt_entry = proc_create("cpufreq_debug",
		...
		mt_entry = proc_create("cpufreq_limited_power",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_cpufreq_dir,
				&mt_cpufreq_limited_power_fops);
		if (!mt_entry)
			pr_err("[%s]:mkdir /proc/cpufreq/cpufreq_limited_power failed\n",
					__FUNCTION__);

		mt_entry = proc_create("cpufreq_state",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_cpufreq_dir,
		...
		mt_entry = proc_create("cpufreq_power_dump",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_cpufreq_dir,
...
#if defined(CONFIG_THERMAL_LIMIT_TEST)
		mt_entry = proc_create("cpufreq_limited_load",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_cpufreq_dir,
				&mt_cpufreq_limited_load_fops);
		if (!mt_entry)
...
#ifdef MT_DVFS_PTPOD_TEST
		mt_entry = proc_create("cpufreq_ptpod_test",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_cpufreq_dir,
				&mt_cpufreq_ptpod_test_fops);
		if (!mt_entry)
			pr_err("[%s]:mkdir /proc/cpufreq/cpufreq_ptpod_test failed\n",
					__FUNCTION__);
#endif

		mt_entry = proc_create("cpufreq_ptpod_freq_volt",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_cpufreq_dir,
...
#ifdef CPUFREQ_SDIO_TRANSFER
		mt_entry = proc_create("cpufreq_sdio_info",
				S_IRUGO | S_IWUSR | S_IWGRP,
				mt_cpufreq_dir,
				&mt_cpufreq_sdio_info_fops);
		if (!mt_entry)
	...
	if (ret) {
		printk(KERN_INFO "Power/DVFS: failed to register cpufreq driver\n");
		return ret;
	} else {
		printk(KERN_INFO "Power/DVFS:cpufreq driver registration done\n");
		printk(KERN_INFO "Power/DVFS:g_cpufreq_get_ptp_level = %d\n",
