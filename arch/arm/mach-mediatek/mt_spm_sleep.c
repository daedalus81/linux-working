...
#define WAKE_SRC_FOR_SUSPEND \
	(WAKE_SRC_KP | WAKE_SRC_EINT 		\
	 | WAKE_SRC_CONN_WDT | WAKE_SRC_IRRX	\
	 | WAKE_SRC_CONN | WAKE_SRC_USB_CD	\
	 | WAKE_SRC_THERM | WAKE_SRC_SYSPWREQ	\
	 | WAKE_SRC_ETHERNET)

#define WAKE_SRC_FOR_DPIDLE \
	(WAKE_SRC_KP | WAKE_SRC_GPT		\
	 | WAKE_SRC_EINT | WAKE_SRC_CONN_WDT	\
	 | WAKE_SRC_IRRX | WAKE_SRC_CONN	\
	 | WAKE_SRC_USB_CD | WAKE_SRC_USB_PDN 	\
	 | WAKE_SRC_AFE | WAKE_SRC_THERM	\
	 | WAKE_SRC_SYSPWREQ | WAKE_SRC_ETHERNET)
...
static void spm_kick_pcm_to_run(bool cpu_pdn, bool infra_pdn, bool pcmwdt_en)
{
	...
	if (pcmwdt_en) {
		...
		spm_write(SPM_PCM_CON1,
				con1 | CON1_CFG_KEY | CON1_PCM_WDT_EN);
	}
#endif
	/* kick PCM to run (only toggle PCM_KICK) */
	con0 = spm_read(SPM_PCM_CON0)
		& ~(CON0_IM_KICK | CON0_PCM_KICK);
...
static void spm_clean_after_wakeup(bool pcmwdt_en)
{
	...
	spm_write(SPM_CLK_CON,
			spm_read(SPM_CLK_CON) & ~CC_SRCLKENA_MASK);
	...
	spm_write(SPM_PCM_CON1, CON1_CFG_KEY
			| (spm_read(SPM_PCM_CON1) & ~CON1_PCM_TIMER_EN));
 ...
 *    >= 0  = specific wakeup period
 */
wake_reason_t spm_go_to_sleep_dpidle(bool cpu_pdn, u16 pwrlevel,
		int pwake_time)
{
	u32 sec = 0;
	int wd_ret;
