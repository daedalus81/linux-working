...
{
	if (pll_id==MT658X_FH_ARM_PLL)  //FHCTL0
		fh_write32(REG_FHCTL0_DDS,
				(fh_read32(ARMPLL_CON1)&0x1FFFFF)|(1U<<31));
	else if (pll_id==MT658X_FH_MAIN_PLL) //FHCTL1
		fh_write32(REG_FHCTL1_DDS,
				(fh_read32(MAINPLL_CON1)&0x1FFFFF)|(1U<<31));
	else if (pll_id==MT658X_FH_MEM_PLL) //FHCTL2
		fh_write32(REG_FHCTL2_DDS,
				((readl(DDRPHY_BASE+0x624) >> 11 )&0x1FFFFF)|(1U<<31));
	else if (pll_id==MT658X_FH_MSDC_PLL) //FHCTL3
		fh_write32(REG_FHCTL3_DDS,
				(fh_read32(MSDCPLL_CON1)&0x1FFFFF)|(1U<<31));
	else if (pll_id==MT658X_FH_MM_PLL) //FHCTL4
		fh_write32(REG_FHCTL4_DDS,
				(fh_read32(MMPLL_CON1)&0x1FFFFF)|(1U<<31));
	else if (pll_id==MT658X_FH_VENC_PLL) //FHCTL5
		fh_write32(REG_FHCTL5_DDS,
				(fh_read32(VENCPLL_CON1)&0x1FFFFF)|(1U<<31));
	/* 8127 FHCTL MB */
	else if (pll_id==MT658X_FH_TVD_PLL) //FHCTL6
		fh_write32(REG_FHCTL6_DDS,
				(fh_read32(TVDPLL_CON1)&0x1FFFFF)|(1U<<31));
	else if (pll_id==MT658X_FH_LVDS_PLL) //FHCTL7
		fh_write32(REG_FHCTL7_DDS,
				(fh_read32(LVDSPLL_CON1)&0x1FFFFF)|(1U<<31));
	/* 8127 FHCTL ME */
	else {
...
static int __mt_enable_freqhopping(unsigned int pll_id,
		const struct freqhopping_ssc* ssc_setting)
{
	...
	//FH_MSG("EN: en_fh");
	FH_MSG("EN: %s:: %x u: %x df: %d dt: %d dds:%x",
			__func__ ,
			ssc_setting->lowbnd
			,ssc_setting->upbnd
			,ssc_setting->df
	...
	//Enable the fh bit
	fh_set_field(REG_FHCTL0_CFG+(0x14*pll_id),
			FH_SFSTRX_DYS,ssc_setting->df);

	fh_set_field(REG_FHCTL0_CFG+(0x14*pll_id),
			FH_SFSTRX_DTS,ssc_setting->dt);
	...
	fh_write32(REG_FHCTL0_UPDNLMT+(pll_id*0x14),
			(PERCENT_TO_DDSLMT((fh_read32(REG_FHCTL0_DDS+(pll_id*0x14)) & 0x1FFFFF),
...
static int __mt_disable_freqhopping(unsigned int pll_id,
		const struct freqhopping_ssc* ssc_setting)
{
	unsigned long 	flags=0;
	//	unsigned int 	pll_hp=0;
...
static int __freqhopping_ctrl(struct freqhopping_ioctl* fh_ctl,bool enable)
{
	...
	if((enable == true) &&
		...
		__mt_disable_freqhopping(fh_ctl->pll_id,pSSC_setting);
			...
			(g_fh_pll[fh_ctl->pll_id].fh_status == FH_FH_DISABLE)) {
	...
	if(enable) { //enable freq. hopping @ fh_ctl->pll_id
		if(g_fh_pll[fh_ctl->pll_id].pll_status == FH_PLL_DISABLE) {
			...
			if(g_fh_pll[fh_ctl->pll_id].user_defined == true) {
				...
				if( g_fh_pll[fh_ctl->pll_id].curr_freq != 0 ) {
					ssc_setting_id =
						g_fh_pll[fh_ctl->pll_id].setting_id =
						__freq_to_index(fh_ctl->pll_id, g_fh_pll[fh_ctl->pll_id].curr_freq);
				} else {
					ssc_setting_id = 0;
				}
...
static int mt_h2oc_mempll(void)
{
	...
	fh_write32( DDRPHY_BASE+0x624,
			(fh_read32(DDRPHY_BASE+0x624) & 0x7FF) ); //clear NCPO
	fh_write32( DDRPHY_BASE+0x624,
			( fh_read32(DDRPHY_BASE+0x624)|(((0x54 << 14) | 0x0A8F ) << 11 )) ); //set NCPO

	//sync NCPO value
	fh_write32(REG_FHCTL2_DDS, (readl(DDRPHY_BASE+0x624) >> 11 )|(1U<<31));
...
static int mt_oc2h_mempll(void)
{
	...
	//sync NCPO value
	fh_write32(REG_FHCTL2_DDS,
			(readl(DDRPHY_BASE+0x624) >> 11 )|(1U<<31));
	...
	fh_write32( DDRPHY_BASE+0x624,
			(fh_read32(DDRPHY_BASE+0x624) & 0x7FF) ); //clear NCPO
	fh_write32( DDRPHY_BASE+0x624,
			( fh_read32(DDRPHY_BASE+0x624) |(((0x4C << 14) | 0x1A2D ) << 11 )) ); //set NCPO
...
static int mt_fh_hal_l2h_mempll(void)  //mempll low to high (200->266MHz)
{
	...
	if(g_fh_pll[MT658X_FH_MEM_PLL].fh_status == FH_FH_ENABLE_SSC){//only when SSC is enable
		...
		while((pll_dds != fh_dds) && ( i < 100)) {
			...
			udelay(10);
			fh_dds  =  readl(REG_FHCTL2_MON) & 0x1FFFFF;
			i++;
		}
		FH_MSG("<p:f:i> %x:%x:%d",pll_dds,fh_dds,i);
	}
...
static int mt_fh_hal_h2l_mempll(void)  //mempll low to high (200->266MHz)
{
	...
	FH_MSG("=> 0x%x", readl(REG_FHCTL2_MON));
	...
	FH_MSG("-MON- 0x%x", readl (REG_FHCTL2_MON));
	...
	fh_write32( DDRPHY_BASE+0x624,
			(fh_read32(DDRPHY_BASE+0x624) & 0x7FF) ); //clear NCPO
	fh_write32( DDRPHY_BASE+0x624,
			( fh_read32(DDRPHY_BASE+0x624) |(((LOW_DRAMC_INT << 14) | LOW_DRAMC_FRACTION ) << 11 )) ); //set NCPO
...
static int mt_fh_hal_dvfs(enum FH_PLL_ID pll_id, unsigned int dds_value)
{
	...
	fh_dds	=  (readl(REG_FHCTL0_MON+(pll_id*0x14))) & 0x1FFFFF;
	...
	if (pll_id==MT658X_FH_ARM_PLL) {
		fh_write32(ARMPLL_CON1,
				(fh_read32(REG_FHCTL0_DVFS)&0x1FFFFF)
				| (fh_read32(ARMPLL_CON1)&0xFFE00000)
				| (1U<<31));
	...
	//FHCTL1
		...
		fh_write32(MAINPLL_CON1,
				(fh_read32(REG_FHCTL1_DVFS)&0x1FFFFF)
				| (fh_read32(MAINPLL_CON1)&0xFFE00000)
				| (1U<<31));
 	...
 	//FHCTL2
		...
		if ((fh_read32(DDRPHY_BASE+0x624))&0x1)
			...
			fh_write32((DDRPHY_BASE+0x624),
					((fh_read32(DDRPHY_BASE+0x624))|0x1));
 	...
 	//FHCTL3
		...
		fh_write32(MSDCPLL_CON1,
				(fh_read32(REG_FHCTL3_DVFS)&0x1FFFFF)
				| (fh_read32(MSDCPLL_CON1)&0xFFE00000)
				| (1U<<31));
 	...
 	//FHCTL4
		...
		fh_write32(MMPLL_CON1,
				(fh_read32(REG_FHCTL4_DVFS)&0x1FFFFF)
				| (fh_read32(MMPLL_CON1)&0xFFE00000)
				| (1U<<31));
	...
	} else if (pll_id==MT658X_FH_VENC_PLL)
	{
		fh_write32(VENCPLL_CON1,
				(fh_read32(REG_FHCTL5_DVFS)&0x1FFFFF)
				| (fh_read32(VENCPLL_CON1)&0xFFE00000)
				| (1U<<31));
 	...
 	//FHCTL3
	{
		fh_write32(TVDPLL_CON1,
				(fh_read32(REG_FHCTL3_DVFS)&0x1FFFFF)
				| (fh_read32(TVDPLL_CON1)&0xFFE00000)
				| (1U<<31));
 	...
 	//FHCTL3
		...
		fh_write32(LVDSPLL_CON1,
				(fh_read32(REG_FHCTL3_DVFS)&0x1FFFFF)
				| (fh_read32(LVDSPLL_CON1)&0xFFE00000)
				| (1U<<31));
...
static int mt_fh_hal_dfs_armpll(unsigned int current_freq, unsigned int target_freq)  //armpll dfs mdoe
{
	...
	target_dds = (((target_freq/100)
				* (((fh_read32(ARMPLL_CON1)&0x1FFFFF)*1000)
				/(current_freq/100)))/1000);
...
static int mt_fh_hal_l2h_dvfs_mempll(void)  //mempll low to high (200->266MHz)
{
	...
	//1pll mode
	if (g_pll_mode==0) {
		if ((readl(DDRPHY_BASE+0x624) >> 11 )==0x14030C) {
	...
	// 3pll mode
		if ((readl(DDRPHY_BASE+0x624) >> 11 )==0x131A2E) {
			FH_MSG("Already @533MHz");
			return 0;
		} else
			FH_MSG("Jump to 533MHz");
	...
	if(g_fh_pll[MT658X_FH_MEM_PLL].fh_status == FH_FH_ENABLE_SSC) {
		...
		fh_write32(REG_FHCTL2_UPDNLMT,
				(PERCENT_TO_DDSLMT((fh_read32(REG_FHCTL2_DDS)&0x1FFFFF),
...
static int mt_fh_hal_h2l_dvfs_mempll(void)  //mempll high to low(266->200MHz)
{
	...
	if (g_pll_mode==0) {
		// 1pll mode
		if ((readl(DDRPHY_BASE+0x624) >> 11 )==0x0F04B0) {
		...
		// 3pll mode
		if ((readl (DDRPHY_BASE+0x624) >> 11 )==0x0E55EE) {
			FH_MSG("Already @400MHz");
			return 0;
		} else
			FH_MSG("Jump to 400MHz");
	...
	if(g_fh_pll[MT658X_FH_MEM_PLL].fh_status == FH_FH_ENABLE_SSC) {
		...
		pll_dds =  (readl(REG_FHCTL2_DDS)) & 0x1FFFFF;
		fh_dds	=  (readl(REG_FHCTL2_MON)) & 0x1FFFFF;
...
static int mt_h2oc_dfs_mempll(void)
{
	...
	FH_MSG("dds: 0x%x",(readl(DDRPHY_BASE+0x624) >> 11 ));

	g_pll_mode = (readl(DDRPHY_BASE+0x60C) & 0x200);
	FH_MSG("g_pll_mode(0:1pll, 1:3pll): %x",g_pll_mode);

	// 1pll mode
	if (g_pll_mode==0) {
		if ((readl(DDRPHY_BASE+0x624) >> 11 )==0x160077) {
	...
	// 3pll mode
		if ((readl(DDRPHY_BASE+0x624) >> 11 )==0x150071) {
			FH_MSG("Already @1172MHz");
			return 0;
		}
		else
...
static void mt_fh_hal_popod_save(void)
{
	...
	if(g_fh_pll[MT658X_FH_MAIN_PLL].fh_status == FH_FH_ENABLE_SSC)	{
		...
		pll_dds =  (readl(REG_FHCTL1_DDS)) & 0x1FFFFF;
		fh_dds	=  (readl(REG_FHCTL1_MON)) & 0x1FFFFF;
...
static void mt_fh_hal_popod_restore(void)
{
	...
	if(g_fh_pll[MT658X_FH_MAIN_PLL].fh_status == FH_FH_ENABLE_SSC) {
		...
		fh_write32(REG_FHCTL1_UPDNLMT,
				(PERCENT_TO_DDSLMT((fh_read32(REG_FHCTL1_DDS)&0x1FFFFF),
...
static int freqhopping_dramc_proc_write(struct file *file, const char *buffer,
...
static int freqhopping_dvfs_proc_read(struct seq_file* m, void* v)
{
	...
	for(i=0;i<MT_FHPLL_MAX;i++) {
		seq_printf(m, "FHCTL%d:   CFG:0x%08x    DVFS:0x%08x\r\n",
				i, readl(REG_FHCTL0_CFG+(i*0x14)),
				readl(REG_FHCTL0_DVFS+(i*0x14)));
	}
...
static int freqhopping_dvfs_proc_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	if (p1==MT658X_FH_MEM_PLL) {
		...
		fh_write32(REG_FHCTL0_UPDNLMT+(p2*0x14),
				(PERCENT_TO_DDSLMT((fh_read32(REG_FHCTL0_DDS+(p2*0x14))&0x1FFFFF),p5) << 16));
...
static int freqhopping_dumpregs_proc_read(struct seq_file* m, void* v)
{
	...
	seq_printf(m, "REG_FHDMA_CFG: 0x%08x 0x%08x 0x%08x 0x%08x\r\n",
			readl(REG_FHDMA_CFG),
			readl(REG_FHDMA_2G1BASE),
			readl(REG_FHDMA_2G2BASE),
			readl(REG_FHDMA_INTMDBASE));

	seq_printf(m, "REG_FHDMA_EXTMDBASE: 0x%08x 0x%08x 0x%08x 0x%08x\r\n",
			readl(REG_FHDMA_EXTMDBASE),
			readl(REG_FHDMA_BTBASE),
			readl(REG_FHDMA_WFBASE),
			readl(REG_FHDMA_FMBASE));

	seq_printf(m, "REG_FHSRAM_CON: 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\r\n",
			readl(REG_FHSRAM_CON),
			readl(REG_FHSRAM_WR),
			readl(REG_FHSRAM_RD),
			readl(REG_FHCTL_CFG),
			readl(REG_FHCTL_CON));
	...
	seq_printf(m, "REG_FHCTL_2G1_CH: 0x%08x 0x%08x 0x%08x 0x%08x\r\n",
			readl(REG_FHCTL_2G1_CH),
			readl(REG_FHCTL_2G2_CH),
			readl(REG_FHCTL_INTMD_CH),
			readl(REG_FHCTL_EXTMD_CH));

	seq_printf(m, "REG_FHCTL_BT_CH: 0x%08x 0x%08x 0x%08x \r\n\r\n",
			readl(REG_FHCTL_BT_CH),
			readl(REG_FHCTL_WF_CH),
			readl(REG_FHCTL_FM_CH));
	...
	for(i=0;i<MT_FHPLL_MAX;i++) {
		...
		seq_printf(m, "0x%08x 0x%08x 0x%08x 0x%08x\r\n",
				readl(REG_FHCTL0_CFG+(i*0x14)),
				readl(REG_FHCTL0_UPDNLMT+(i*0x14)),
				readl(REG_FHCTL0_DDS+(i*0x14)),
				readl(REG_FHCTL0_MON+(i*0x14)));
	}

	seq_printf(m, "\r\nPLL_HP_CON0:\r\n0x%08x\r\n",
			readl(PLL_HP_CON0));

	seq_printf(m, "\r\nPLL_CON0 :\r\nARM:0x%08x MAIN:0x%08x MSDC:0x%08x"
			"MM:0x%08x VENC:0x%08x\r\n",
			readl(ARMPLL_CON0),
			readl(MAINPLL_CON0),
			readl(MSDCPLL_CON0),
			readl(MMPLL_CON0),
	...
	seq_printf(m, "\r\nMEMPLL :\r\nMEMPLL9: 0x%08x MEMPLL10: 0x%08x"
			"MEMPLL11: 0x%08x MEMPLL12: 0x%08x\r\n",
			readl(DDRPHY_BASE+0x624),
			readl(DDRPHY_BASE+0x628),
			readl(DDRPHY_BASE+0x62C),
			readl(DDRPHY_BASE+0x630)); //TODO: Hard code for now...
	seq_printf(m, "\r\nMEMPLL :\r\nMEMPLL13: 0x%08x MEMPLL14: 0x%08x"
			"MEMPLL15: 0x%08x MEMPLL16: 0x%08x\r\n",
			readl(DDRPHY_BASE+0x634),
			readl(DDRPHY_BASE+0x638),
			readl(DDRPHY_BASE+0x63C),
			readl(DDRPHY_BASE+0x640)); //TODO: Hard code for now...
...
static int freqhopping_clkgen_proc_read(struct seq_file* m, void* v)
{
	...
	seq_printf(m, "\r\nMBIST :\r\nMBIST_CFG_2: 0x%08x"
			"MBIST_CFG_6: 0x%08x MBIST_CFG_7: 0x%08x\r\n",
			readl(MBIST_CFG_2),
			readl(MBIST_CFG_6),
			readl(MBIST_CFG_7));
	...
	seq_printf(m, "\r\nGPIO: 0x%08x 0x%08x 0x%08x 0x%08x\r\n",
			readl(GPIO_BASE+0xC60),
			readl(GPIO_BASE+0xC70),
			readl(GPIO_BASE+0xCD0),
			readl(GPIO_BASE+0xD90));


	seq_printf(m, "\r\nDDRPHY_BASE :\r\n0x%08x 0x%08x 0x%08x"
			"0x%08x 0x%08x 0x%08x\r\n",
			readl(DDRPHY_BASE+0x600),
			readl(DDRPHY_BASE+0x604),
			readl(DDRPHY_BASE+0x608),
			readl(DDRPHY_BASE+0x60C),
			readl(DDRPHY_BASE+0x614),
			readl(DDRPHY_BASE+0x61C));
#if 0
	*start = page + off;
...
static int freqhopping_clkgen_proc_write(struct file *file, const char *buffer,
		unsigned long count, void *data)
{
	int len = 0, pll_id = 0;
	char clkgen[32];
	...
	if (sscanf(clkgen, "%d", &pll_id) == 1)	{
		if(pll_id == MT658X_FH_ARM_PLL){
			...
			fh_write32(DDRPHY_BASE+0x600,
					((readl(DDRPHY_BASE+0x600)) | 0x1<<5));

			fh_write32(DDRPHY_BASE+0x60C,
					((readl(DDRPHY_BASE+0x60C)) | 0x1<<21));
			fh_write32(DDRPHY_BASE+0x614,
					((readl(DDRPHY_BASE+0x614)) | 0x1<<21));
			fh_write32(DDRPHY_BASE+0x61C,
					((readl(DDRPHY_BASE+0x61C)) | 0x1<<21));

			fh_write32(DDRPHY_BASE+0x60C,
					((readl(DDRPHY_BASE+0x60C)) & ~0x7));
			fh_write32(DDRPHY_BASE+0x60C,
					((readl(DDRPHY_BASE+0x60C)) | 0x2 ));
			fh_write32(DDRPHY_BASE+0x614,
					((readl(DDRPHY_BASE+0x614)) & ~0x7));
			fh_write32(DDRPHY_BASE+0x614,
					((readl(DDRPHY_BASE+0x614)) | 0x2 ));
			fh_write32(DDRPHY_BASE+0x61C,
					((readl(DDRPHY_BASE+0x61C)) & ~0x7));
			fh_write32(DDRPHY_BASE+0x61C,
					((readl(DDRPHY_BASE+0x61C)) | 0x2));

			fh_write32(DDRPHY_BASE+0x604,
					((readl(DDRPHY_BASE+0x604)) | 0x1<<3));
			fh_write32(DDRPHY_BASE+0x604,
					((readl(DDRPHY_BASE+0x604)) | 0x1<<7));
			fh_write32(DDRPHY_BASE+0x604,
					((readl(DDRPHY_BASE+0x604)) | 0x1<<4));
			fh_write32(DDRPHY_BASE+0x604,
					((readl(DDRPHY_BASE+0x604)) | 0x1<<9));

#if 0
			fh_write32(DDRPHY_BASE+0x608,
					((readl(DDRPHY_BASE+0x608)) & ~0x000E0000));
#endif
			fh_write32(DDRPHY_BASE+0x608,
					((readl(DDRPHY_BASE+0x608)) | 0x00040000 ));

			fh_write32(DDRPHY_BASE+0x608,
					((readl(DDRPHY_BASE+0x608)) & ~0xF0000000 ));
			fh_write32(DDRPHY_BASE+0x608,
					((readl(DDRPHY_BASE+0x608)) | 0x80000000 ));
