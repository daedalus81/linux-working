...
static int mt_hotplug_mechanism_test0_show(struct seq_file *s, void *v)
{
	...
	printk(KERN_EMERG "CPU%u, debug event: 0x%08x, debug monitor: 0x%08x\n", 0,
			readl(0xf0200080), readl(0xf0200084));
...
static int mt_hotplug_mechanism_test0_write(struct file *file,
...
static int mt_hotplug_mechanism_test1_show(struct seq_file *s, void *v)
{
...
#if 0
  	/* These variables are not public in cpufreq_hotplug.c */
	...
	HOTPLUG_INFO("[power/hotplug] thermal_debug_2 (%d)(%d)(%d)(%d)\n",
			g_cpu_up_count, g_cpu_up_load_index,
			g_cpu_down_count, g_cpu_down_load_index);
#endif

	return 0;
}

static int mt_hotplug_mechanism_test1_open(struct inode *inode,
		struct file *file)
{
	return single_open(file, mt_hotplug_mechanism_test1_show, NULL);
}

static int mt_hotplug_mechanism_test1_write(struct file *file,
		const char *buffer, size_t count, loff_t *data)
{
	int len = 0, test1 = 0;
	char desc[32];
...
static int __init mt_hotplug_mechanism_init(void)
{
	...
	{
		entry = proc_create("test0", S_IRUGO | S_IWUSR,
				mt_hotplug_dir,
		...
		entry = proc_create("test1", S_IRUGO | S_IWUSR,
				mt_hotplug_dir,
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	mt_hotplug_mechanism_early_suspend_handler.suspend =
		mt_hotplug_mechanism_early_suspend;

	mt_hotplug_mechanism_early_suspend_handler.resume =
