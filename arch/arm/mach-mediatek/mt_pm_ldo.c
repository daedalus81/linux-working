...
bool hwPowerOn(MT65XX_POWER powerId,
		MT65XX_POWER_VOLTAGE powerVolt, char *mode_name)
{
	UINT32 i = 0;
	int j=0, k=0;
	...
	if(first_power_on_flag == 1) {
		for(j=0 ; j<MT65XX_POWER_COUNT_END ; j++) {
			for(k=0 ; k<MAX_DEVICE ; k++)
				sprintf(g_MT_PMIC_BusHW.Power[j].mod_name[k],
						"%s", NON_OP);

			g_MT_PMIC_BusHW.Power[j].dwPowerCount=0;
		}
		first_power_on_flag = 0;
		printk(KERN_INFO "Power/PMIC", "[hwPowerOn] init done.\r\n");
	}
	...
	for (i = 0; i< MAX_DEVICE; i++)	{
		printk(KERN_INFO "Power/PMIC", "[hwPowerOn] %d,%s,%d\r\n",
				i, g_MT_PMIC_BusHW.Power[powerId].mod_name[i],
				g_MT_PMIC_BusHW.Power[powerId].dwPowerCount);

		if (!strcmp(g_MT_PMIC_BusHW.Power[powerId].mod_name[i], NON_OP)) {
			MSG(PMIC,"[%s] acquire powerId:%d index:%d"
					"mod_name: %s powerVolt:%d\r\n",
					__FUNCTION__, powerId, i,
					mode_name,powerVolt);

			sprintf(g_MT_PMIC_BusHW.Power[powerId].mod_name[i] ,
					"%s", mode_name);
			break ;
		}
		/* already it */
	...
	if(g_MT_PMIC_BusHW.Power[powerId].dwPowerCount > 1) {
		printk(KERN_INFO , "Power/PMIC:[hwPowerOn]"
				"g_MT_PMIC_BusHW.Power[powerId].dwPowerCount (%d) > 1\r\n",
				g_MT_PMIC_BusHW.Power[powerId].dwPowerCount);
		return TRUE;
	}
#endif
	...
	printk(KERN_INFO, "Power/PMIC: [hwPowerOn] enable %d by %s \r\n",
			powerId, mode_name);
...
bool hwPowerDown(MT65XX_POWER powerId, char *mode_name)
{
	...
	for (i = 0; i< MAX_DEVICE; i++)	{
		printk(KERN_INFO "Power/PMIC:[hwPowerDown] %d,%s,%d\r\n",
				i, g_MT_PMIC_BusHW.Power[powerId].mod_name[i],
				g_MT_PMIC_BusHW.Power[powerId].dwPowerCount);
	...
	g_MT_PMIC_BusHW.Power[powerId].dwPowerCount--;
	if(g_MT_PMIC_BusHW.Power[powerId].dwPowerCount > 0) {
		printk(KERN_INFO "Power/PMIC:[hwPowerDown] g_MT_PMIC_BusHW."
				"Power[powerId].dwPowerCount (%d) > 0\r\n",
				g_MT_PMIC_BusHW.Power[powerId].dwPowerCount);
		return TRUE;
	}
	...
	printk(KERN_INFO "Power/PMIC:[hwPowerDown] disable %d by %s \r\n",
			powerId, mode_name);
